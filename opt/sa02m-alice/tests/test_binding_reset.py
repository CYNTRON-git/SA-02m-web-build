...
Since 1.0.6.32 the behaviour under test lives in the SHARED binding-reset core
(`sa02m_alice/common/binding_core.py`, byte-identical to the cloud agent's
authoritative copy) driven through THIS door's descriptor
(`sa02m_alice/common/binding_sources.py`). So the assertions below hold the
core to the Alice door's contract, and the cloud agent's own suite holds it to
the other door's — a fail-open edit inside the core fails on both at once.
...
class _BudgetExhausted(BaseException):
    ...
    That is only HALF the problem. A budget must also be REACHABLE: a budget
    hanging off `_stop.wait` and the clock is stepped around by a mutation that
    produces a hot loop touching neither. `_BindingBase.cert_ceiling` is the
    reachability half — see it for which call every loop must make.
...
from sa02m_alice.common import binding_sources  # noqa: E402
...
class _BindingBase(unittest.TestCase):
    def setUp(self):
        ...
        # The unlinked flag, the counter and the pending-wipe record are
        # process-global by design (one client process per board); tests must
        # not inherit them from each other.
        self.reset_globals()
    ...
    def cert_ceiling(self, limit=400):
        ...
        `_await_cert`. So a ceiling here cannot be stepped around by a mutation
        that produces a hot loop touching neither `_stop.wait` nor the clock.

        The inner watchdog loop is the one loop that never asks about
        certificates; the `_Clock` tick budget covers that one instead. Between
        them every loop in `client/main.py` has a reachable ceiling.
        """
    ...
    @staticmethod
    def spec(write_status=None):
        return binding_sources.yandex_source(write_status)

    def reset(self, reason=None, write_status=None):
        """The whole routine, exactly as both doors run it."""
        return binding_core.stand_down(
            self.spec(write_status),
            C.REFUSAL_CLASS_UNLINKED,
            C.UNLINK_REFUSAL if reason is None else reason,
        )

    @staticmethod
    def wipe():
        """The wipe alone, for the assertions about WHAT it touches."""
        return binding_core.wipe_binding(binding_sources.binding_files)

    @staticmethod
    def marker():
        return config_store.unlink_marker()
    ...
    def binding_present(self):
        return sorted(
            os.path.basename(p)
            for p in [C.CERT_FILE, C.KEY_FILE, C.CA_FILE, C.PENDING_CLAIM_FILE,
                      C.KEY_FILE + ".tmp"]
            if os.path.exists(p)
        )
...
class TestResetCloudBinding(_BindingBase):
    ...
    def test_writes_the_durable_marker_into_client_conf(self):
        self.seed_binding()
        self.assertEqual(self.reset(), "repair")
        stamp, cls, reason = self.marker()
        self.assertTrue(stamp.endswith("Z"), stamp)
        self.assertEqual(cls, C.REFUSAL_CLASS_UNLINKED)
        self.assertEqual(reason, C.UNLINK_REFUSAL)
        text = self.read(C.CLIENT_CONF).decode("utf-8")
        for key in C.UNLINK_MARKER_KEYS:
            self.assertIn(key, text)

    def test_the_marker_is_the_same_three_keys_the_cloud_door_writes(self):
        """One vocabulary, two INIs: the cloud agent writes `unlinked_at` /
        `unlinked_reason` / `unlinked_reason_text` into agent.conf and this door
        writes the same three into the client INI, so the restore-after-reboot
        rule is written once in the core rather than twice."""
        self.assertEqual(
            C.UNLINK_MARKER_KEYS,
    ...
    def test_sidecar_glob_cannot_reach_the_gateway_ca(self):
        self.seed_binding()
        sidecars = binding_sources._sidecars()
        self.assertEqual([os.path.basename(p) for p in sidecars], ["device.key.pem.tmp"])
        self.assertNotIn(C.CA_FILE, sidecars)

    def test_the_stand_down_status_carries_the_shared_vocabulary(self):
        self.seed_binding()
        written = []
        self.reset(write_status=lambda state, **kw: written.append((state, kw)))
        state, kw = written[-1]
        self.assertEqual(state, C.STATE_UNLINKED)
        self.assertEqual(kw["reason_class"], C.REFUSAL_CLASS_UNLINKED)
        self.assertEqual(kw["reason"], C.UNLINK_REFUSAL)
        self.assertEqual(kw["unlinked_at"], self.marker()[0])


# =====================================================================
# The threshold: a VERDICT, not evidence — the parameter that is not shared
# =====================================================================
class TestVerdictThreshold(_BindingBase):
    """The number differs from the cloud door's THREE on purpose.

    This door receives one delivered verdict on a verified mTLS session and the
    gateway disconnects right after sending it, so a second event never arrives
    in the same session: waiting for three would mean waiting forever, which is
    the 1.0.6.27 bug back. Setting this descriptor's threshold to 3 must turn
    the first test below RED.
    """

    def test_the_alice_descriptor_counts_to_one(self):
        self.assertEqual(self.spec().threshold, 1)

    def test_a_single_unlink_event_reaches_the_verdict(self):
        tracker = binding_core.tracker_for(self.spec())
        self.assertEqual(
            binding_core.refusal_verdict(self.spec(), tracker, C.EVT_CONTROLLER_UNLINK),
            C.REFUSAL_CLASS_UNLINKED,
        )

    def test_a_single_unlink_event_erases_the_binding_end_to_end(self):
        self.seed_binding()
        handled = []
        ...
        handlers = SioHandlers(
            ...
            on_unlink=lambda: handled.append(self.reset()),
        )
        handlers.handle(C.EVT_CONTROLLER_UNLINK, {"reason": "unlinked"})
        self.assertEqual(handled, ["repair"])
        self.assertEqual(self.binding_present(), [])

    def test_nothing_but_the_unlink_event_classifies_as_a_refusal(self):
        for evidence in (None, "", {}, "disconnect", "connect", 403, 500,
                         "controller_unlin", "Controller_Unlink", []):
            self.assertEqual(
                binding_sources.classify_unlink(evidence), "",
                "%r classified as a refusal" % (evidence,),
            )

    def test_this_door_may_not_write_the_revoke_state(self):
        """`revoked` is N/A here: the gateway's event does not distinguish an
        owner revoke from a detach and the handler reads no payload field. The
        descriptor declares only the two states it may write, and the core
        refuses anything else rather than putting a state on a card that has no
        label for it."""
        self.assertEqual(self.spec().states, (C.STATE_UNLINKED, C.STATE_UNLINK_FAILED))
        bad = self.spec()._replace(state_for_class=lambda cls: "revoked")
        self.seed_binding()
        with self.assertRaises(ValueError):
            binding_core.stand_down(bad, C.REFUSAL_CLASS_UNLINKED, C.UNLINK_REFUSAL)
...
class TestFailedWipeIsNeverReportedAsDone(_BindingBase):
    ...
    to the suite until this class existed.
    """
    ...
    def test_the_failure_is_logged_before_it_propagates(self):
        ...
        with mock.patch("os.unlink", side_effect=PermissionError(13, "denied")):
            with self.assertLogs("sa02m.binding", level="ERROR") as cm:
                with self.assertRaises(PermissionError):
                    self.wipe()
        self.assertTrue(any("could not remove" in line for line in cm.output))

    def test_a_failed_wipe_is_an_explicit_error_state_never_done(self):
        """The guarantee this door GAINED in 1.0.6.32.

        Before it, the handler caught the OSError, logged it and returned — so
        a wipe that failed on a read-only filesystem left the card saying
        «привязан» on a board the gateway had already unbound: the exact false-
        card class this feature exists to kill, one layer down.
        """
        self.seed_binding()
        written = []
        with mock.patch("os.unlink", side_effect=PermissionError(13, "denied")):
            rc = self.reset(write_status=lambda state, **kw: written.append((state, kw)))
        self.assertEqual(rc, C.STATE_UNLINK_FAILED)
        state, kw = written[-1]
        self.assertEqual(state, C.STATE_UNLINK_FAILED)
        self.assertEqual(kw["reason"], "wipe_failed")
        self.assertIn("denied", kw["detail"])
        self.assertEqual(kw["reason_class"], C.REFUSAL_CLASS_UNLINKED)
        # Still on disk, and NOT recorded as unlinked.
        self.assertNotEqual(self.binding_present(), [])
        self.assertEqual(self.marker()[0], "")
        self.assertEqual(binding_core.PENDING_STAND_DOWN["cls"], C.REFUSAL_CLASS_UNLINKED)

    def test_the_retry_ladder_finishes_the_stand_down_once_the_wipe_succeeds(self):
        self.seed_binding()
        written = []
        writer = lambda state, **kw: written.append((state, kw))  # noqa: E731
        with mock.patch("os.unlink", side_effect=PermissionError(13, "denied")):
            self.assertEqual(self.reset(write_status=writer), C.STATE_UNLINK_FAILED)
            # Still failing: the retry keeps the error state, never "done".
            self.assertEqual(binding_core.retry_wipe(self.spec(writer)), "")
            self.assertEqual(written[-1][0], C.STATE_UNLINK_FAILED)
        # The permission problem goes away.
        self.assertEqual(binding_core.retry_wipe(self.spec(writer)), "repair")
        self.assertEqual(self.binding_present(), [])
        self.assertEqual(written[-1][0], C.STATE_UNLINKED)
        self.assertTrue(self.marker()[0])
        self.assertEqual(binding_core.PENDING_STAND_DOWN, {"cls": "", "reason": ""})

    def test_a_pending_failed_wipe_outranks_every_other_status_write(self):
        """While the binding is still on disk the only honest card is «Ошибка
        отвязки» — an `offline` landing on top of it would read as an ordinary
        disconnect on a board that is in trouble."""
        binding_core.PENDING_STAND_DOWN.update({"cls": "unlinked", "reason": "x"})
        for state in (C.STATE_OFFLINE, C.STATE_ERROR, C.STATE_DISABLED,
                      C.STATE_CONNECTED, C.STATE_UNLINKED):
            self.assertTrue(client_main._suppress_status(state, False, True), state)
        self.assertFalse(
            client_main._suppress_status(C.STATE_UNLINK_FAILED, False, True))


# =====================================================================
# The cross-PROCESS hand-off: a failed wipe on the CGI path
# =====================================================================
class TestFailedWipeIsHandedToTheClient(_BindingBase):
    """The local button runs in a CGI that answers and exits.

    Its in-memory pending record dies with it, and `/run/sa02m-alice` is
    0755 root:root (etc/tmpfiles.d/sa02m-alice.conf) so it cannot even write the
    status file. Without a durable hand-off the certificates stay on disk, the
    client keeps dialling a gateway that has already unlinked us, and the card
    keeps reading the board as bound - the 1.0.6.27 loop one layer down, on the
    very control this change makes visible.

    Every test here asserts the hand-off is REAL, not announced.
    """

    def fail_the_wipe(self):
        return mock.patch("os.unlink", side_effect=OSError(30, "Read-only file system"))

    def cgi_unlink(self):
        """The local button, exactly as sa02m_alice_api.cgi runs it."""
        with mock.patch.object(api, "probe_gateway", return_value=PROBE_UP),                 mock.patch.object(api.urllib.request, "urlopen", return_value=_Resp()):
            return api.unlink_controller()

    def test_a_failed_cgi_wipe_leaves_a_durable_record_for_another_process(self):
        self.seed_binding()
        with self.fail_the_wipe():
            result = self.cgi_unlink()
        self.assertFalse(result["ok"])
        self.assertEqual(result["error"], "wipe_failed")
        # The record is on DISK, in the one file both processes can write.
        self.assertEqual(
            binding_sources.read_pending(),
            (C.REFUSAL_CLASS_UNLINKED, binding_sources.SOURCE_LOCAL))
        self.assertIn("unlinked_reason", self.read(C.CLIENT_CONF).decode("utf-8"))

    def test_the_pending_record_never_claims_the_erase_happened(self):
        """`unlinked_at` stays EMPTY while the files are still there - that empty
        stamp is what keeps the terminal flag off a still-bound board."""
        self.seed_binding()
        with self.fail_the_wipe():
            self.cgi_unlink()
        self.assertEqual(self.marker()[0], "")
    ...
    def test_the_client_adopts_the_record_and_finishes_the_job(self):
        """The finding, closed end to end: a client process with nothing in
        memory (exactly like a restart) picks the stand-down up and erases."""
        self.seed_binding()
        with self.fail_the_wipe():
            self.cgi_unlink()
        self.reset_globals()
        self.assertFalse(client_main._wipe_pending())

        written = []
        spec = self.spec(lambda state, **kw: written.append((state, kw)))
        self.assertTrue(client_main._retry_pending_wipe(spec))

        self.assertEqual(self.binding_present(), [])
        self.assertTrue(client_main._unlinked.is_set())
        self.assertTrue(self.marker()[0])
        self.assertEqual(written[-1][0], C.STATE_UNLINKED)
        # And the record is spent - a later tick must not re-adopt it.
        self.assertEqual(binding_sources.read_pending(), ("", ""))
        self.assertFalse(client_main._retry_pending_wipe(spec))

    def test_while_the_wipe_keeps_failing_the_client_says_so_on_the_card(self):
        """The status half of the promise: the CGI cannot write the status file,
        so the CLIENT is what puts the error state on the card."""
        self.seed_binding()
        with self.fail_the_wipe():
            self.cgi_unlink()
            self.reset_globals()
            ...
            self.assertFalse(client_main._retry_pending_wipe(spec))
        self.assertEqual(written[-1][0], C.STATE_UNLINK_FAILED)
        self.assertEqual(written[-1][1]["reason"], "wipe_failed")
        self.assertTrue(client_main._wipe_pending())

    def test_a_record_that_could_not_be_written_is_reported_not_claimed(self):
        """The honest second outcome: when the hand-off ALSO fails, the answer
        says nothing is retrying rather than promising a mechanism."""
        self.seed_binding()
        with self.fail_the_wipe(),                 mock.patch.object(binding_sources, "set_unlink_marker",
                                  side_effect=OSError(30, "Read-only file system")):
            result = self.cgi_unlink()
        self.assertFalse(result["ok"])
        self.assertEqual(result["error"], "wipe_failed_not_recorded")
        self.assertEqual(binding_sources.read_pending(), ("", ""))

    def test_the_hand_off_failing_does_not_turn_an_error_into_a_crash(self):
        """Best effort by design: this path runs where a disk write has already
        failed once, so a second failure must not escape as an exception."""
        self.seed_binding()
        with self.fail_the_wipe(),                 mock.patch.object(binding_sources, "set_unlink_marker",
                                  side_effect=OSError(30, "Read-only file system")):
            rc = self.reset()
        self.assertEqual(rc, C.STATE_UNLINK_FAILED)
        # The in-process record still stands - only the hand-off was lost.
        self.assertTrue(client_main._wipe_pending())

    def test_adopting_never_overrides_a_stand_down_this_process_owns(self):
        binding_core.PENDING_STAND_DOWN.update({"cls": "mine", "reason": "in flight"})
        config_store.set_unlink_marker("", "unlinked", "someone else")
        self.assertFalse(binding_core.adopt_pending(self.spec()))
        self.assertEqual(binding_core.PENDING_STAND_DOWN["cls"], "mine")

    def test_nothing_is_adopted_on_a_healthy_board(self):
        self.seed_binding()
        self.assertFalse(binding_core.adopt_pending(self.spec()))
        self.assertFalse(client_main._retry_pending_wipe(self.spec()))

    def test_a_completed_marker_is_not_a_pending_one(self):
        """The completed marker overwrites the pending record in one write -
        there is no separate key to leave behind and no clear to forget."""
        self.seed_binding()
        self.assertEqual(self.reset(), "repair")
        self.assertTrue(self.marker()[0])
        self.assertEqual(binding_sources.read_pending(), ("", ""))

    def test_the_journal_does_not_claim_an_erase_that_failed(self):
        """The line that opens a stand-down is printed BEFORE the wipe, so it
        must be imperative; the past tense belongs to the success path."""
        self.seed_binding()
        with self.assertLogs("sa02m.binding", level="ERROR") as cm:
            with self.fail_the_wipe():
                self.reset()
        joined = NL.join(cm.output)
        self.assertNotIn("binding erased", joined)
        self.assertIn("erasing the binding", joined)
...
class TestN4Idempotent(_BindingBase):
    ...
    def test_a_second_unlink_event_re_runs_an_idempotent_reset(self):
        """The journal cursor is N/A on this door — an event that arrives again
        on a later reconnect is made harmless by idempotence instead: every file
        reports `absent` and the marker write is a set, not an append."""
        self.seed_binding()
        self.assertEqual(self.reset(), "repair")
        first = self.marker()
        self.assertEqual(self.reset(), "repair")
        ...
        text = self.read(C.CLIENT_CONF).decode("utf-8")
        self.assertEqual(text.count("unlinked_at"), 1)
...
class TestN5Boundary(_BindingBase):
    def test_wipe_touches_the_cloud_binding_and_nothing_else(self):
        """The plan's blast-radius paragraph turned into a test.

        devices.conf and server.conf must come out BYTE-IDENTICAL. client.conf
        cannot be byte-identical — the durable marker is three keys inside it —
        so the assertion there is per key: every pre-existing key keeps its
        value, the three marker keys are the ONLY addition, and client_enabled
        stays ON (switching it off would hide the link button the next owner
        needs).
        """
        self.seed_binding()
        ...
        # `cloud_control_enabled` is not an addition of the wipe: save_ini
        # writes the section it parsed, and default_client_cfg supplies that
        # key's documented default — the same thing every existing writer
        # (set_client_enabled, set_cloud_control) already materialises.
        self.assertEqual(
            sorted(k for k, _ in cfg.items("client")),
            sorted(["client_enabled", "cloud_control_enabled", "log_level",
                    "mqtt_host", "mqtt_port"] + list(C.UNLINK_MARKER_KEYS)),
        )
        self.assertEqual(cfg.get("client", "cloud_control_enabled"), "false")

    def test_the_clear_list_is_exactly_the_four_binding_files(self):
        self.assertEqual(
            sorted(os.path.basename(p) for p in binding_sources.binding_files()),
            ["ca.crt.pem", "device.crt.pem", "device.key.pem", "pending_claim.json"],
        )
        for path in binding_sources.binding_files():
            self.assertTrue(
                os.path.abspath(path).startswith(os.path.abspath(C.VAR_DIR)),
                "%s escapes the identity dir" % path,
            )

    def test_the_device_document_is_not_in_the_clear_list(self):
        """A re-bound board must find the same devices. Adding DEVICES_CONF to
        the erase-list is the obvious wrong widening; it turns this RED and the
        byte-identity assertion above with it."""
        for path in binding_sources.binding_files():
            self.assertNotEqual(os.path.abspath(path), os.path.abspath(C.DEVICES_CONF))
            self.assertNotEqual(os.path.abspath(path), os.path.abspath(C.SERVER_CONF))
            self.assertNotEqual(os.path.abspath(path), os.path.abspath(C.CLIENT_CONF))
...
class TestHandlerUnlinkBranch(_BindingBase):
    def make(self, on_unlink=None, profile=C.PROFILE_YANDEX):
        registry = mock.MagicMock()
    ...
    def test_the_cloud_profile_never_stands_down(self):
        """The named non-goal: sa02m-cloud-control holds no identity of its own
        — it authenticates with the cloud agent's device_id + device_secret, and
        the agent's own stand-down erases those. Two writers to one binding is
        the over-count class reborn."""
        rec = _Recorder()
        h = self.make(rec, profile=C.PROFILE_CLOUD)
        for payload in ({"reason": "unlinked"}, {}, None):
            h.handle(C.EVT_CONTROLLER_UNLINK, payload)
        self.assertEqual(rec.calls, 0)
...
class TestStatusSuppression(_BindingBase):
    def test_suppression_rule_truth_table(self):
        for state in (
            ...
            self.assertFalse(client_main._suppress_status(state, False, False), state)
            self.assertTrue(client_main._suppress_status(state, True, False), state)
        self.assertFalse(client_main._suppress_status(C.STATE_UNLINKED, True, False))
        self.assertFalse(client_main._suppress_status(C.STATE_UNLINKED, False, False))
    ...
    def test_unlinked_status_still_carries_the_reload_handshake_flag(self):
        ...
        every state; the new states must not be the exception, or the web
        trigger loses its skip-the-restart evidence."""
        for state in (C.STATE_UNLINKED, C.STATE_UNLINK_FAILED):
            self.reset_globals()
            if state == C.STATE_UNLINK_FAILED:
                binding_core.PENDING_STAND_DOWN.update({"cls": "unlinked", "reason": "x"})
            client_main._write_status(state, message=C.UNLINKED_MESSAGE)
            data = self.status()
            self.assertIs(data["config_watch"], True, state)
            self.assertIs(data["cert_present"], False, state)
...
class TestCertWaitRouting(_BindingBase):
    ...
    def test_should_wait_for_cert_truth_table(self):
        """The unlink stop must NOT hang off needs_cert.
    ...
    def test_marker_written_by_the_cgi_is_adopted_by_the_client_loop(self):
        config_store.set_unlink_marker("2026-09-02T10:00:00Z", "unlinked", "controller_unlink")
        client_main._reconcile_unlink_state()
        self.assertTrue(client_main._unlinked.is_set())

    def test_a_certificate_reappearing_lifts_the_flag_here_too(self):
        """The lift may not live only inside _await_cert.
        ...
        Without this the flag stays terminal and the card is frozen at
        «unlinked» on a board that is connected and working.
        """
        client_main._unlinked.set()
        self.write(C.CERT_FILE, "c")
    ...
    def test_await_cert_returns_on_rebind_and_clears_the_terminal_flag(self):
        ...
        self.write(C.KEY_FILE, "k")
        self.assertTrue(
            client_main._await_cert(C.PROFILE_YANDEX, "m", "https://gw", None))
        self.assertFalse(client_main._unlinked.is_set())
    ...
    def test_await_cert_retries_a_pending_failed_wipe(self):
        """A partial wipe failure (the key gone, the certificate not) lands in
        the soft wait with the retry still pending; the card must follow what
        the retry does rather than freeze."""
        self.cert_ceiling()
        self.write(C.CA_FILE, "a")
        binding_core.PENDING_STAND_DOWN.update(
            {"cls": C.REFUSAL_CLASS_UNLINKED, "reason": C.UNLINK_REFUSAL})
        _waits, tick = _budget(20, "the no-certificate soft wait")
        ...
        with mock.patch.object(client_main._stop, "wait", side_effect=fake_wait):
            self.assertFalse(
                client_main._await_cert(C.PROFILE_YANDEX, "m", "https://gw", self.spec()))
        self.assertEqual(binding_core.PENDING_STAND_DOWN, {"cls": "", "reason": ""})
        self.assertTrue(client_main._unlinked.is_set())
        self.assertTrue(self.marker()[0])
...
class TestN1ErrorPathWipesNothing(_BindingBase):
    def test_connect_failures_never_erase_the_binding(self):
        ...
        before = {
            p: self.read(p)
            for p in binding_sources.binding_files()
        }
...
class _UnlinkingSio:
    ...
    on_unlink -> the shared core -> the status write and the loop exit.
    """
...
class TestFailedWipeNeverMarksTheBoardUnlinked(_BindingBase):
    """The caller half of B2.

    on_unlink must not mark the board unlinked on a failed erase: no terminal
    flag, no «отвязано» card. What it MUST do — and this is what 1.0.6.27's
    handler did not — is say so: `unlink_failed` on the card and a retry ladder,
    never a swallowed OSError that leaves «привязан» on an unbound board.
    """

    def test_a_failed_erase_says_so_and_never_claims_the_board_is_unlinked(self):
        self.cert_ceiling()
        self.seed_binding()
        _UnlinkingSio.instances = 0
        waits = {"n": 0}
        ...
        with mock.patch.object(client_main, "AliceSocketIO", _UnlinkingSio), \
                ...
                mock.patch.object(client_main, "time", _Clock(budget)), \
                mock.patch.object(sio_connection, "import_socketio", return_value=None), \
                mock.patch.object(binding_core, "wipe_binding",
        ...
        self.assertFalse(client_main._unlinked.is_set())
        self.assertEqual(self.marker()[0], "")
        # …and the card says the honest thing rather than «привязан».
        self.assertEqual(self.status()["state"], C.STATE_UNLINK_FAILED)
        self.assertEqual(self.status()["reason"], "wipe_failed")


class TestUnlinkStopsTheLoopOnWsToo(_BindingBase):
    """The stop and the flag-lift must not hang off needs_cert.

    On a lab ws:// gateway needs_cert is False. Gated behind it, the board keeps
    dialling after the wipe (12 dials measured where the wss:// test asserts 1),
    and `_unlinked` stays terminal for the life of the process because
    `_await_cert` is one of only two `_unlinked.clear()` sites.
    """
    ...
    def _budgets(self, on_second_wait=None, on_wait=2, stop_after_ticks=None):
        ...
        suite ends up hanging with no named failure.
        """
        ...
        def budget(_s=None):
            n = ctick()
            # Scripted end of a HEALTHY session: _UnlinkOnceSio stays connected
            # after a re-bind, so without this the watchdog loop would
    ...
    def test_a_rebind_during_the_backoff_still_unfreezes_the_card(self):
        """Through the real loop: the certificate comes back on the FIRST wait
        — the post-disconnect backoff — so the outer loop never enters the soft
        wait at all. Without the lift there, 13 dials were measured with the
        status frozen at «unlinked» on a bound board; the card must reach
        `connected`."""
        self.cert_ceiling()
        self.use_ws_gateway()
        self.seed_binding()
        ...
        with mock.patch.object(client_main, "AliceSocketIO", _UnlinkOnceSio), \
                mock.patch.object(client_main, "_mqtt_client", return_value=mock.MagicMock()), \
                mock.patch.object(client_main, "reconnect_delay", return_value=0.0), \
                mock.patch.object(client_main, "time", _Clock(budget)), \
                mock.patch.object(sio_connection, "import_socketio", return_value=None), \
...
class TestLocalUnlinkSuccess(_BindingBase):
    ...
    def test_the_local_button_no_longer_switches_the_client_off(self):
        """The Operator-visible change: `client_enabled` stays ON, so the link
        row (and its «Привязать» button) stays on the card and «отвязать →
        привязать заново» works without SSH. Clearing the flag hides the row
        (app/alice.js gates it on the client being enabled)."""
        self.seed_binding()
        with mock.patch.object(api, "probe_gateway", return_value=PROBE_UP), \
                mock.patch.object(api.urllib.request, "urlopen", return_value=_Resp()):
    ...
    def test_a_failed_wipe_on_the_local_path_is_reported_not_claimed_done(self):
        self.seed_binding()
        with mock.patch.object(api, "probe_gateway", return_value=PROBE_UP), \
                mock.patch.object(api.urllib.request, "urlopen", return_value=_Resp()), \
                mock.patch("os.unlink", side_effect=PermissionError(13, "denied")):
            result = api.unlink_controller()
        self.assertFalse(result["ok"])
        self.assertEqual(result["error"], "wipe_failed")
        self.assertNotEqual(self.binding_present(), [])
...
class TestRebindClearsTheMarker(_BindingBase):
    ...
    def test_a_failed_rebind_does_not_clear_the_marker(self):
        ...
        self.assertEqual(self.marker()[0], "2026-09-02T10:00:00Z")
...
class TestMarkerStore(_BindingBase):
    def test_set_and_clear_round_trip(self):
        ...
        config_store.set_unlink_marker("2026-09-02T10:00:00Z", "unlinked", "controller_unlink")
        self.assertEqual(
            self.marker(), ("2026-09-02T10:00:00Z", "unlinked", "controller_unlink"))
        config_store.clear_unlink_marker()
        self.assertEqual(self.marker(), ("", "", ""))
        text = self.read(C.CLIENT_CONF).decode("utf-8")
        for key in C.UNLINK_MARKER_KEYS:
