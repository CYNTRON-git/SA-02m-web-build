...
- [OPEN] 2026-08-21 **[LOW] EN tooltips go stale after a re-poll (`i18n.js`).**
  ...
  that branch's fence.
- [OPEN] 2026-08-24 **[MED] eth1 (port 2) has no DHCP path in the panel — 2-eth boards can't use DHCP on port 2.** `apply.cgi` eth1 branch only writes static or retires; `forms.js`/`index.html` give port 2 an enable+static toggle with no static/DHCP mode selector; `config.cgi` can't distinguish DHCP from disabled. Installer default IS `inet dhcp` but the panel destroys it on first touch. HARDWARE-CONFIRMED 2026-08-24 on the 2-eth bench board: writing `iface eth1 inet dhcp` by hand → eth1 got 192.168.1.66 from the Keenetic immediately. Operator decision recorded: make port 2 a toggle like port 1 (on=static, off=DHCP). Plan `.ai-dev/plans/eth1-dhcp-panel.md`. NEXT BRANCH.
- [OPEN] 2026-08-24 **[HIGH] OTA deploys extension-less helpers as 0644 → panel buttons die.** Full analysis in `private/BUG-ota-mode-0644.md`. `sa02m-update-runner.sh` derives file mode from the SOURCE extension, so `etc/sa02m-set-storage-auto-format` and `etc/sa02m-set-cpu-profile` (no `.sh`) land non-executable on every web update: exFAT autoformat button dies («НЕ УСТАНОВЛЕНО»), CPU-profile switching fails silently. Installer + offline map both get it right (offline map's explicit `"mode":"0755"` is the in-repo reference). Fix: derive mode from DESTINATION (`/usr/local/sbin` → executable) + regression test. Modes restored by hand on the bench; golden image carries them correct.
