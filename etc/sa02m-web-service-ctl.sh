...
nodered_install_online() {
    ...
    rm -f "$_tmp"
    # armhf + Node 20 cannot run Node-RED v4/v5 → pin v3 (mirrors 07:86-104).
    ...
    if [ "$_arch" = "armhf" ] || [ "$_arch" = "armv7l" ]; then
        ...
        if [ -n "$_nodemajor" ] && [ "$_nodemajor" -lt 22 ] 2>/dev/null; then
            _nrver=$(npm ls -g node-red --parseable=false 2>/dev/null | awk -F@ '/node-red@/{print $NF; exit}')
            _nrmajor="${_nrver%%.*}"
            if [ -n "$_nrmajor" ] && [ "$_nrmajor" -ge 4 ] 2>/dev/null; then
                npm install -g --no-audit --no-fund --unsafe-perm node-red@3 >>"$LOG" 2>&1 || true
            fi
        fi
    fi
    _nr_home=$(getent passwd nodered 2>/dev/null | cut -d: -f6)
