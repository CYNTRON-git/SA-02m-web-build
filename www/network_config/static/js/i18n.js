...
(function () {
  ...
  const DICT = {
    ...
    'Недопустимое название': 'Invalid name',
    ...
    'Устройства для животных': 'Pet devices',
    'Климатическая техника': 'Climate appliances',
    'Электрооборудование': 'Electrical',
    'Открытие/закрытие': 'Openable',
    'Остальные устройства': 'Other devices',
    'Умная кнопка': 'Smart button',
    'Датчик газа': 'Gas sensor',
    'Датчик освещённости': 'Illumination sensor',
    'Датчик открытия двери': 'Door sensor',
    'Датчик дыма': 'Smoke sensor',
    'Датчик вибрации': 'Vibration sensor',
    'Датчик протечки воды': 'Water leak sensor',
    'Счётчик холодной воды': 'Cold-water meter',
    'Счётчик газа': 'Gas meter',
    'Счётчик тепла': 'Heat meter',
    'Счётчик горячей воды': 'Hot-water meter',
    'Видеокамера': 'Camera',
    'Медиаустройство': 'Media device',
    'Ресивер': 'Receiver',
    'Телевизор': 'TV',
    'ТВ-приставка': 'TV box',
    ...
    'Люстра': 'Ceiling light',
    'Диммер': 'Dimmer',
    'Гирлянда': 'Garland',
    'Настольная лампа': 'Desk lamp',
    'Бра': 'Sconce',
    'Диодная лента': 'LED strip',
    'Торшер': 'Floor lamp',
    'Реле': 'Relay',
    'Открываемое': 'Openable',
    'Шторы': 'Curtains',
    ...
    'Нет зависимостей': 'Dependencies missing',
  ...
  'Вентустановка': 'Air handling unit',
  'Климат': 'Climate',
  'Уставка температуры': 'Temperature setpoint',
  'Температура притока': 'Supply air temperature',
  'Температура обратной воды': 'Return water temperature',
  'Температура в помещении': 'Room temperature',
  'Температура снаружи': 'Outdoor temperature',
  ...
  'Установка вкл.': 'Unit on',
    ...
    'выход активен нулём': 'output is active-low',
    'Добавить показание': 'Add reading',
    'Удалить показание': 'Remove reading',
    'Вид показания': 'Reading kind',
    'Давление': 'Pressure',
    'Углекислый газ': 'Carbon dioxide',
    'Летучие вещества': 'Volatile compounds',
    'Движение': 'Motion',
    'показания': 'readings',
    'показаний': 'readings',
    ...
    'Два показания одного вида в одном устройстве — выберите разные':
      'Two readings of the same kind in one device — choose different ones',
    ...
    'Доступ отозван': 'Access revoked',
    'Отвязано в облаке': 'Unlinked in the cloud',
    'Привязать заново': 'Pair again',
    'Причина': 'Reason',
    'доступ отозван владельцем': 'access revoked by the owner',
    'устройство отвязано в облаке': 'device unlinked in the cloud',
    'облако не признаёт устройство': 'the cloud does not recognise the device',
    ...
    'Доступ по сети': 'Network Access',
    'Внимание: включённый порт принимает подключения с любого компьютера в сети — без пароля. Ограничьте доступ полем «Разрешённые адреса» ниже.': 'Warning: an enabled port accepts connections from any computer on the network — with no password. Narrow it with the “Allowed addresses” field below.',
    'Адрес прослушивания': 'Listening Address',
    '0.0.0.0 — все сетевые интерфейсы платы': '0.0.0.0 — every network interface of the board',
    'Разрешённые адреса': 'Allowed Addresses',
    'Пусто — подключаться может любой. Через запятую: адреса и диапазоны, которым разрешено подключаться к этому порту.': 'Empty — anyone may connect. Comma-separated: the addresses and ranges allowed to reach this port.',
    ...
    // ── Carel c.pCOmini / uAria: окно настройки приточной установки (1.0.6.31) ──
    // RU/EN пары взяты из десктопного прошивальщика (ветка carel, _i18n_carel.py),
    // чтобы два интерфейса одной установки говорили одними словами.
    'Приточная установка Carel': 'Carel air handling unit',
    'Настройка приточной установки': 'Air Handler Configuration',
    'Приточная установка': 'Air handling unit',
    'Исполнение платы': 'Board variant',
    'Установка опознана ответом на функцию 17: регистр сигнатуры и серийный номер контроллер Carel не отдаёт.': 'The unit is identified by its Report Slave ID (function 17) reply: a Carel controller answers neither the signature register nor the serial number.',
    'Установка': 'Plant',
    'Входы/выходы': 'I/O',
    'Тревоги': 'Alarms',
    'Параметры линии (только просмотр)': 'Line parameters (read-only)',
    'Смена адреса, скорости и чётности — с клавиатуры контроллера (uAria: Hd01–Hd03; c.pCO: Sv01–Sv05), по сети эти параметры не пишутся.': 'The slave address, baud rate and parity are changed from the controller keypad (uAria: Hd01–Hd03; c.pCO: Sv01–Sv05); they are never written over the bus.',
    'Остановлена': 'Stopped',
    'Клапан нагрева': 'Heat valve',
    'Команды и уставки': 'Commands and setpoints',
    'Уставка зима, °C': 'Winter setpoint, °C',
    'Уставка лето, °C': 'Summer setpoint, °C',
    'Приточный вентилятор, %': 'Supply fan, %',
    'Вытяжной вентилятор, %': 'Exhaust fan, %',
    'Скорость вентилятора, ступень 1–10': 'Fan speed, step 1–10',
    '0 - Выключено': '0 - Off',
    '1 - Включено': '1 - On',
    '2 - Расписание': '2 - Schedule',
    '3 - Цифровой вход': '3 - Digital input',
    '4 - Расписание и цифровой вход': '4 - Schedule + DI',
    '5 - th-Tune': '5 - th-Tune',
    'Разрешение запуска по сети': 'Allow network start',
    'Запуск по сети': 'Network start',
    'Расчётный выход': 'Calculated output',
    'Фактический AO': 'Actual AO',
    'Локальный терминал (только чтение)': 'Local terminal (read-only)',
    'Сезон': 'Season',
    'Применить уставки': 'Apply setpoints',
    'Карта uAria: пуск по сети — катушка 0, разрешение Gs04 — катушка 13. Катушка 30 принадлежит местному терминалу и из веба не пишется.': 'uAria map: network start is coil 0, the Gs04 enable is coil 13. Coil 30 belongs to the local terminal and is never written from the web.',
    'Аналоговые входы (датчики)': 'Analog inputs (sensors)',
    'Дискретные выходы (реле)': 'Digital outputs (relays)',
    'Цифровые входы': 'Digital inputs',
    'Аналоговые выходы': 'Analog outputs',
    'Показаны переменные карты BMS (функции программы) и их состояния. Номера клемм по сети не читаются: что на какой клемме — задаёт мастер входов/выходов в самом ПЛК.': 'The BMS map variables (program functions) and their states are shown. Terminal numbers cannot be read over the bus: the assignment is made by the I/O wizard inside the PLC.',
    'Нет активных тревог': 'No active alarms',
    'Сброс тревог': 'Reset alarms',
    'Сброс — импульс 5 с на катушке сброса. Активный контакт на клемме останется, и тревога поднимется снова.': 'The reset is a 5 s pulse on the reset coil. A contact that is still active at the terminal will raise the alarm again.',
    // Тосты и confirm() окна установки (confirm обёрнут t(): нативный диалог
    // недоступен наблюдателю).
    'Изменённых уставок нет': 'No setpoint changes',
    'Уставки сохранены': 'Setpoints saved',
    'Команда пуска отправлена': 'Start command sent',
    'Команда останова отправлена': 'Stop command sent',
    'Сброс тревог выполнен': 'Alarms reset',
    'Разрешение сети записано': 'Network start enable written',
    'Режим работы записан': 'Operating mode written',
    'Включить установку по сети (катушка 0)? При необходимости сначала уйдёт Gs04 (катушка 13).': 'Start the air handler over the network (coil 0)? Gs04 (coil 13) is written first if it is off.',
    'Включить установку по сети? Уйдут разрешение Ma18 (катушка 130), выдержка и команда пуска (катушка 65).': 'Start the air handler over the network? Ma18 (coil 130), the settle delay and the start command (coil 65) are written in that order.',
    'Остановить установку по сети? Разрешение сети и пуск с клавиатуры остаются как были.': 'Stop the air handler over the network? The network enable and the keypad start stay as they are.',
    'Сбросить тревоги (импульс 5 с)? Активный контакт на клемме останется.': 'Reset alarms (a 5 s pulse)? A contact that is still active at the terminal will remain.',
    // ── LED strip window (1.0.6.40) — static/js/flasher/led.js ──────────────
    // RU/EN пары взяты из десктопного прошивальщика (ветка feat/led-spy-and-picker,
    // _i18n_rgbw.py), чтобы два интерфейса одной ленты говорили одними словами;
    // RU-ключи модуля (LED_T) — там же. Расхождения с десктопом: «ЧЧ/ММ/СС» вместо
    // «Часы/Минуты/Секунды» (RU «Часы» — заголовок карточки), «Огонь и жар» для
    // группы (RU «Огонь» — эффект 26), «Скорость эффекта», «Без чётности», «Выкл».
    'RGBW каналы': 'RGBW channels',
    'Запись выполнена': 'Write completed',
    'Параметры PWM записаны': 'PWM settings written',
    'Параметры DI записаны': 'DI settings written',
    'Параметры адресной ленты записаны': 'Addressable strip settings written',
    'Параметры сцены записаны': 'Scene settings written',
    'Адресная лента': 'Addressable strip',
    'Сцена': 'Scene',
    'Входы DI': 'DI inputs',
    '1 канал': '1 channel',
    '2 канала': '2 channels',
    'APA102 (SPI, 1 канал)': 'APA102 (SPI, 1 channel)',
    'Пул пикселей': 'Pixel pool',
    'Встроенный FX': 'Built-in FX',
    'Эффект Flash': 'Flash effect',
    'Силовые RGBW (PWM)': 'Power RGBW (PWM)',
    'Канал R': 'Channel R',
    'Канал G': 'Channel G',
    'Канал B': 'Channel B',
    'Канал W': 'Channel W',
    ...
    'CCT2 WW': 'CCT2 WW',
    'CCT2 CW': 'CCT2 CW',
    '4×W (уровень)': '4×W (level)',
    '4×W (параллель)': '4×W (parallel)',
    '2×W (A)': '2×W (A)',
    '2×W (A∥)': '2×W (A∥)',
    '2×W (B)': '2×W (B)',
    '2×W (B∥)': '2×W (B∥)',
    ...
    'W + W + 2×W': 'W + W + 2×W',
    '2×CCT': '2×CCT',
    'Тип светодиода': 'LED type',
    'Порядок цветов': 'Color order',
    'Автообновление': 'Auto refresh',
    'Светодиодов на ленте': 'LEDs on strip',
    'Светодиодов на ленте 2': 'LEDs on strip 2',
    'Формат пикселя': 'Pixel format',
    'RGB': 'RGB',
    'RGBW': 'RGBW',
    'Некорректное значение': 'Invalid value',
    'Источник сцены': 'Scene source',
    'Эффект': 'Effect',
    'Скорость эффекта': 'Effect speed',
    'Яркость FX': 'FX brightness',
    'Статичный': 'Static',
    'Дыхание': 'Breath',
    'Радуга': 'Rainbow',
    'Радуга по ленте': 'Rainbow cycle',
    'Радуга-2D': 'Rainbow 2D',
    'Игра цветов': 'Color wheel',
    'Праздник': 'Party',
    'Аврора': 'Aurora',
    'Цветовые волны': 'Color waves',
    'Спиннер': 'Spinner',
    'Конфетти': 'Confetti',
    'Матрица: плазма': 'Matrix: plasma',
    'Матрица: дождь': 'Matrix: rain',
    'Ливень': 'Downpour',
    'Капля': 'Droplet',
    'Дождь по воде': 'Rain on water',
    'Мыльные пузыри': 'Soap bubbles',
    'Круги-всплески': 'Splash rings',
    'Океан': 'Ocean',
    ...
    'Сканер': 'Scanner',
    'Pac-Man': 'Pac-Man',
    'Заливка цветом': 'Color wipe',
    'Заливка к центру': 'Fill to center',
    'Заполнение бегом': 'Running fill',
    'Сопровождение': 'Escort',
    'Театральный': 'Theater',
    'Змейка (игра)': 'Snake (game)',
    'Искры (заполнение)': 'Sparks (fill)',
    'Снегопад': 'Snowfall',
    'Снежинки (калейдоскоп)': 'Snowflakes (kaleidoscope)',
    'Молния': 'Lightning',
    'Летний лес': 'Summer forest',
    'Осенний лес': 'Autumn forest',
    'Падающая звезда': 'Falling star',
    'Ураган': 'Hurricane',
    'Матрица: текст': 'Matrix: text',
    'Часы HH:MM': 'Clock HH:MM',
    'Метеостанция': 'Weather station',
    'Логотип ЦИНТРОН': 'CYNTRON logo',
    'Стрелка': 'Arrow',
    'Шеврон': 'Chevron',
    'Знак «Внимание»': 'Warning sign',
    'Знак STOP': 'STOP sign',
    '«ВЫХОД»': '"EXIT"',
    '«ПОЖАР»': '"FIRE"',
    '«ТРЕВОГА»': '"ALARM"',
    ...
    '«РОССИЯ»': '"RUSSIA"',
    'Снаряд': 'Projectile',
    'Стрела Амура': 'Cupid\'s arrow',
    'Метеориты': 'Meteors',
    'Индикатор линии': 'Line indicator',
    'База и заливка': 'Base & solid',
    'Цветные и радуга': 'Colorful & rainbow',
    'Вода': 'Water',
    'Огонь и жар': 'Fire & heat',
    'Небо и свет': 'Sky & light',
    'Физика и геометрия': 'Physics & geometry',
    'Природа и погода': 'Nature & weather scenes',
    'Текст и цифры': 'Text & numeric',
    'Указатели и таблички': 'Signs & indication',
    'Специальные': 'Special',
    'Вариант/направление': 'Variant/direction',
    'Цвет эффекта': 'Effect color',
    ...
    'Стиль анимации текста': 'Text animation style',
    'Направление': 'Direction',
    'Процент': 'Percent',
    'Другая ориентация': 'Alternate orientation',
    'Цвет пятна': 'Light-pool color',
    'Обратное направление': 'Reverse direction',
    'Длина пятна, px (0 = 60)': 'Pool length, px (0 = 60)',
    'Остановить полосы': 'Freeze the stripes',
    'В двухстрочном режиме (Строк = 2) работает только стиль «Титры»': 'With Lines = 2 only the "Credits" style renders',
    'Авто (статично или бегущая строка)': 'Auto (static or scrolling)',
    'Печатная машинка (мигающий курсор)': 'Typewriter (blinking cursor)',
    'Проявление по буквам (fade-in)': 'Per-letter fade-in',
    'Влёт букв справа по одной': 'Letters fly in from the right, one by one',
    'Волна (буквы качаются ±1 px)': 'Wave (letters sway ±1 px)',
    'Мигание 1 Гц': 'Blink 1 Hz',
    'Всплытие букв снизу': 'Letters float up from below',
    'Дешифровка (глифы оседают в текст)': 'Decrypt (glyphs settle into the text)',
    'Глитч (сдвиги-двойники)': 'Glitch (shifted ghosts)',
    'Неон (случайная буква притухает)': 'Neon (a random letter dims out)',
    'Падение сверху с отскоками': 'Fall from above with bounces',
    'Растворение по пикселям': 'Pixel dissolve',
    'Радуга (подменяет цвет)': 'Rainbow (overrides the color)',
    'Проезд насквозь (справа → пауза → влево)': 'Drive-through (in from the right → pause → out left)',
    'Разлёт на частицы и кольца': 'Explode into particles and rings',
    'Морфинг «A|B» (перетекание слова A в B)': 'Morph "A|B" (word A flows into B)',
    'Титры (вертикальная прокрутка)': 'Credits (vertical scroll)',
    'Дрожь (джиттер ±1 px)': 'Jitter (±1 px)',
    'Стиль «Морфинг» требует разделитель | в тексте (без него — растворение)': 'The "Morph" style needs a | separator in the text (without it: dissolve)',
    'Текст шире полотна всегда идёт обычной бегущей строкой, независимо от стиля': 'Text wider than the canvas always falls back to a plain marquee',
    'Вправо': 'Right',
    'Влево': 'Left',
    'Вверх': 'Up',
    'Вниз': 'Down',
    'Плотность': 'Density',
    'Гамма': 'Gamma',
    'Прогрессивная развёртка': 'Progressive scan',
    'Начало снизу': 'Start from bottom',
    'Тип матрицы': 'Panel type',
    '8×8 (ш8×в8)': '8×8 (w8×h8)',
    '8×16 (ш8×в16)': '8×16 (w8×h16)',
    '16×8 (ш16×в8)': '16×8 (w16×h8)',
    '16×16 (ш16×в16)': '16×16 (w16×h16)',
    '8×32 (ш8×в32)': '8×32 (w8×h32)',
    '32×8 (ш32×в8)': '32×8 (w32×h8)',
    '16×32 (ш16×в32)': '16×32 (w16×h32)',
    '32×16 (ш32×в16)': '32×16 (w32×h16)',
    '32×32 (ш32×в32)': '32×32 (w32×h32)',
    '64×16 (ш64×в16)': '64×16 (w64×h16)',
    'Зеркально по X': 'Mirror X',
    'Столбцами (swap XY)': 'Column-major (swap XY)',
    'Число матриц': 'Tile count',
    'Авто': 'Auto',
    '1': '1',
    '2': '2',
    '3': '3',
    '4': '4',
    'Эффект на матрицах': 'Effect across tiles',
    'Растянуть на всё полотно': 'Span the whole canvas',
    'Повторить на каждой': 'Repeat on each tile',
    'Зеркально (калейдоскоп)': 'Mirrored (kaleidoscope)',
    'Канал 2': 'Channel 2',
    'Общее для обоих каналов и раздельно НЕ настраивается: тип светодиода, формат пикселя, порядок цветов, гамма, эффект, скорость и яркость. Например, WS2812 на канале 1 и SK6812 на канале 2 одновременно невозможны. Своё у канала 2 только два параметра — длина и режим. Буфер пикселей тоже общий: (длина 1 + длина 2) × 3 байта (RGB) или × 4 (RGBW) не больше 3072.': 'Shared by both channels and NOT configurable per channel: LED type, pixel format, color order, gamma, effect, speed and brightness. WS2812 on channel 1 with SK6812 on channel 2, for instance, is impossible. Channel 2 owns exactly two settings — its length and its mode. The pixel buffer is shared too: (length 1 + length 2) × 3 bytes (RGB) or × 4 (RGBW) must not exceed 3072.',
    'Что показывает канал 2: «Синхронно» и «Зеркально» — ту же картинку, что канал 1 (зеркально — в обратном порядке); длина канала 2 в этих режимах не используется. «Продолжение полотна» — одно общее полотно, канал 2 продолжает канал 1. «Независимо» — свой участок буфера, но эффекты рисуют только канал 1: эти пиксели пишет мастер по Modbus (пул с адреса 1100 + 2 × длина канала 1), сам модуль их не формирует. Нужны эффекты на обоих выходах — «Синхронно», «Зеркально» или «Продолжение полотна».': 'What channel 2 shows: "Sync" and "Mirror" show the same picture as channel 1 (mirrored = reversed order); the channel-2 length is unused in those modes. "Canvas continuation" makes one canvas that channel 2 continues. "Independent" gives it its own slice of the buffer, but the effects render channel 1 only: those pixels are written by the master over Modbus (pool from 1100 + 2 × channel-1 length), the module does not generate them. For effects on both outputs use Sync, Mirror or Canvas continuation.',
    'Режим канала 2': 'Channel 2 mode',
    'Синхронно (та же картинка)': 'Sync (same picture)',
    'Независимо (пиксели от мастера)': 'Independent (pixels from the master)',
    'Зеркально (та же картинка наоборот)': 'Mirror (same picture reversed)',
    'Продолжение полотна': 'Canvas continuation',
    'Бегущая строка': 'Marquee text',
    'Часы': 'Clock',
    'Рабочий порт': 'Work port',
    'Выкл (оба слейва)': 'Off (both slaves)',
    'Порт A (торцевая)': 'Port A (end-face)',
    'Порт B (внешний)': 'Port B (aux)',
    'Режим порта': 'Port mode',
    'Spy (подслушка)': 'Spy (listen)',
    'Master (опрос)': 'Master (poll)',
    'Подслушка на слейв-порту': 'Tap on slave port',
    'Скорость WorkPort': 'WorkPort baud',
    'Без чётности': 'None',
    'Чёт': 'Even',
    'Нечет': 'Odd',
    'Период опроса, мс': 'Poll period, ms',
    'Таймаут ответа, мс': 'Reply timeout, ms',
    'Устаревание, мс': 'Stale after, ms',
    'Значения': 'Live values',
    'Параметры прослушки': 'Listen parameters',
    'Адреса прослушки': 'Listen addresses',
    'Слот': 'Slot',
    'FC': 'FC',
    'Регистр': 'Register',
    ...
    'Низ': 'Lo',
    'Верх': 'Hi',
    'lo=hi (в т.ч. 0/0) — выкл. Ниже — синий, выше — красный, в диапазоне — цвет 463/464': 'lo=hi (incl. 0/0) = off. Below = blue, above = red, in range = colour 463/464',
    '03 Holding': '03 Holding',
    '04 Input': '04 Input',
    '06 Write single': '06 Write single',
    '16 Write multi': '16 Write multi',
    'INT16': 'INT16',
    'UINT16': 'UINT16',
    'INT32 AB': 'INT32 AB',
    'UINT32 AB': 'UINT32 AB',
    'INT32 CDAB': 'INT32 CDAB',
    'UINT32 CDAB': 'UINT32 CDAB',
    'FLOAT ABCD (BE)': 'FLOAT ABCD (BE)',
    'FLOAT CDAB': 'FLOAT CDAB',
    'FLOAT BADC': 'FLOAT BADC',
    'FLOAT DCBA (LE)': 'FLOAT DCBA (LE)',
    'Время ЧЧ': 'Time HH',
    'Время ММ': 'Time MM',
    'Время СС': 'Time SS',
    'Тип «Время ЧЧ/ММ/СС» (10/11/12) собирает одни часы: ЧЧ+ММ+СС → 14:05:09, без секунд → 14:05, без часов → 05:09. Адрес 0 — поле не задано и на ленту не попадает. Двоеточие мигает как в режиме 64. Остальные слоты — обычные значения. 2 строки: часы сверху, значения снизу.': 'Time HH/MM/SS (types 10/11/12) compose one clock: HH+MM+SS → 14:05:09, no seconds → 14:05, no hours → 05:09. UID 0 omits that field from the strip. Colon blinks as in mode 64. Other slots stay normal values. Two lines: clock on top, values below.',
    'Прослушка полей': 'Listen binds',
    'UID 0 — поле не показывается. Если задан хотя бы один UID, лента берёт только прослушку (локальные 453/457–462 игнорируются). Нужен WorkPort (карта «Индикатор линии»). Не сохраняется при выключении.': 'UID 0 hides the field. If any UID is set, the strip uses listen only (local 453/457–462 ignored). Needs WorkPort (Line indicator card). Not saved across power loss.',
    'ЧЧ': 'HH',
    'ММ': 'MM',
    'СС': 'SS',
    'Дата': 'Date',
    'Цвета текста': 'Text colors',
    'Шрифт 5×7 — только ЗАГЛАВНЫЕ; текст приводится к верхнему регистру': 'The 5×7 font is UPPERCASE only — text is folded to upper case',
    'Размер текста': 'Text size',
    'Одна строка, крупный шрифт 2x (10×14)': 'One line, large 2x font (10×14)',
    'Две строки, мелкий 5×7 (разделитель |)': 'Two lines, small 5×7 (| separator)',
    'Крупный 2x — только пока текст целиком влезает в ширину полотна (13 px на символ, на 64 px помещается 5 символов); иначе прошивка сама переходит на 5×7 и прокручивает.': 'The large 2x font is used only while the whole text fits the canvas width (13 px per character; on a 64 px canvas that is 5 characters); beyond that the firmware falls back to 5×7 and scrolls.',
    'Цвет текста 1': 'Text color 1',
    'Цвет текста 2': 'Text color 2',
    'Фон текста 1': 'Text background 1',
    'Фон текста 2': 'Text background 2',
    'Год': 'Year',
    'Цвет температуры': 'Temperature color',
    'Вывод значений': 'Value layout',
    '1 строка': '1 line',
    '2 строки': '2 lines',
    'Время с ПК': 'Time from PC',
    'Слот Flash': 'Flash slot',
    'Цикл': 'Loop',
    'Загрузить Flash': 'Load Flash',
    'Ток, мА': 'Current, mA',
    'NTC платы, °C': 'Board NTC, °C',
    'VLED, В': 'VLED, V',
    'Светодиодная лента LED': 'LED strip',
    'Настройка светодиодной ленты': 'LED strip configuration',
    'Лента не отвечает — проверьте линию и адрес': 'The strip does not answer — check the line and the address',
    'Демон прошивальщика старее веб-интерфейса (нет списков выбора) — обновите пакет sa02m-flasher': 'The flasher daemon is older than the web interface (no choice lists) — update the sa02m-flasher package',
    'Телеметрия': 'Telemetry',
    'Выходы ленты': 'Strip outputs',
    'Развёртка матрицы': 'Matrix wiring',
    'Группа эффектов': 'Effect group',
    'Текст': 'Text',
    'Записать текст': 'Write text',
    'Записать': 'Write',
    'Часы и минуты задаются вместе': 'Hours and minutes are set together',
    'Дождитесь завершения предыдущей команды': 'Wait for the previous command to finish',
    ...
    'Ошибка защиты сессии — повторите действие': 'Session protection error — please repeat the action',
  };
  ...
  const REGEX = [
    ...
    [/^Ethernet № 2: DHCP — конфиг сохранён, адрес запрашивается…$/u, 'Ethernet #2: DHCP — config saved, address requesting…'],
    ...
    [/^Укажите IP и маску для Ethernet № 2$/u, 'Enter IP and subnet mask for Ethernet #2'],
