...
function renderServicesControl(data) {
  ...
  sorted.forEach(function (svc, i) {
    ...
    // Matrix: not installed → [Установить]; installed+running → [Стоп][Удалить];
    // installed+stopped → [Пуск][Удалить].
    if (!installed) {
      mid.appendChild(makeSvcCtlButton(svc, 'install', flasherBusy));
    } else {
      mid.appendChild(makeSvcCtlButton(svc, svcCtlWantsStart(svc) ? 'start' : 'stop', flasherBusy));
      mid.appendChild(makeSvcCtlButton(svc, 'uninstall', flasherBusy));
    }
