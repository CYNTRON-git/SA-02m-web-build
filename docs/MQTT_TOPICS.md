...
# Управляемые выходы PCA9536 (R/W) — соответствие битам см. ниже
...
### Дискретный выход, пищалка и светодиод — соответствие битам (1.0.6.42)

Три канала `do` / `beeper` / `alarm_led` управляют расширителем **PCA9536**
(шина I2C 2, адрес `0x41`, регистр вывода `0x01`). Соответствие «канал → бит»
и полярность задаются **только** в `/etc/sa02m_hw.conf` — это единственный дом
этих значений. Их читают и веб-интерфейс (`cgi-bin/lib_hw.sh`), и демон
телеметрии (`sa02m_telemetry.py`); в коде они не продублированы.

| Канал | Ключ конфигурации | Бит на плате | Что коммутирует |
|---|---|---|---|
| `alarm_led` | `SA02M_I2C_BIT_ALARM_LED` | 0 | светодиод «Авария» |
| `do` | `SA02M_I2C_BIT_DO` | 1 | **дискретный выход** |
| `beeper` | `SA02M_I2C_BIT_BEEPER` | 2 | зуммер |
| — | `SA02M_I2C_EXTRA_OUTPUT_MASK` | 3 | синий светодиод KLogic (веб не управляет) |

**Полярность — активный НОЛЬ.** При `SA02M_I2C_ACTIVE_LOW_MASK=auto` (значение
по умолчанию) все объявленные выходы считаются активными по нулю: логическая
`1` в топике **сбрасывает** бит в регистре, `0` — устанавливает. Поэтому
«всё выключено» читается из регистра как `0x0F`.

В топиках `/devices/SA-02m/controls/*` всегда публикуется **логический**
уровень (`1` = включено), а не сырой бит. Флаг `inverted` в профиле Яндекс
Алисы для этих трёх каналов **не нужен**
(`docs/contracts/alice-mqtt-mapping.md` § Inverted).

`SA02M_HW_BACKEND` демон соблюдает: при `disabled` он не обращается к шине
вовсе, при `gpio_sysfs` — отказывается управлять выходами (этот бэкенд
реализован только в веб-интерфейсе) и пишет об этом в журнал. Канал, для
которого бит в конфигурации отсутствует или не разбирается, **отклоняется с
записью в журнал** и никогда не управляется по угаданному биту.

### Занятая шина: `beeper` проходит, `do` и `alarm_led` — нет (1.0.6.43)

Расширитель делят четыре владельца: демон телеметрии, веб-интерфейс, воркер
пищалки и MPLC4/KLogic. Пока шину держит владелец из `SA02M_I2C_OWNER_UNITS` /
`SA02M_I2C_OWNER_PROCS`, демон **не пишет** в неё — команда отклоняется с
записью в журнал (с 1.0.6.42). С 1.0.6.43 из этого правила ровно одно
исключение, и оно повторяет поведение кнопки в веб-панели:

| Команда при занятой шине | Что происходит |
|---|---|
| `beeper/on` | **принимается**: демон пишет файл-перекрытие `SA02M_BEEPER_OVERRIDE_FILE` (`value=<0\|1>`, `expires_at=<сейчас + SA02M_BEEPER_WEB_OVERRIDE_SEC>`, по умолчанию 7 с) и запускает `SA02M_BEEPER_OVERRIDE_WORKER`. Воркер применяет значение к биту зуммера под тем же flock. В топик публикуется заданное значение, в журнале — строка со словом `override` и именем владельца шины |
| `do/on` | отклоняется (как и было): дискретный выход коммутирует реальное оборудование |
| `alarm_led/on` | отклоняется (как и было): это постоянный индикатор, а не импульс на 7 с |

**Что здесь изменилось для внешнего клиента.** Команда `beeper` на занятой
шине раньше возвращалась отказом в журнале и ничего не публиковала — теперь
она успешна: зуммер звучит, а в топик уходит заданное значение. Клиент,
который считал такой отказ признаком «плата занята», больше его не увидит.
`do` и `alarm_led` не изменились.

Файл-перекрытие с 1.0.6.43 пишут **двое** — веб-интерфейс и демон. Запись
всегда атомарна (временное имя + переименование), поэтому воркер никогда не
прочитает половину файла; побеждает последний писавший, со своим сроком
`expires_at`. Демон не перечитывает собственную запись и **не удаляет** файл:
срок жизни перекрытия определяет только `expires_at`, который проверяет воркер.
Если воркер не установлен, файл всё равно записан и команда считается принятой
— это поведение веб-панели, повторённое дословно.

Одно отличие от веб-панели, намеренно в сторону строгости: канал без
разобранного бита в `/etc/sa02m_hw.conf` демон отклоняет и здесь, тогда как
веб-панель ушла бы в перекрытие и воркер применил бы его к своему запасному
биту 2. Управлять угаданным битом — это и есть дефект 1.0.6.42.

> **История.** До 1.0.6.42 демон телеметрии хранил своё соответствие каналов
> битам в коде (`do`→0, `beeper`→1, `alarm_led`→2) и не читал
> `/etc/sa02m_hw.conf`. Соответствие было сдвинуто относительно платы, а запись
> велась без учёта активного нуля, поэтому команда `beeper` переключала
> **дискретный выход**, а опубликованное состояние было обратным реальному.
> Профили, где на эти каналы был выставлен `inverted: true` в обход дефекта,
> нужно поправить — флаг снять.
...
Дом таблицы — `MR02M_MODULE_TYPES` / `MR02M_TYPE_NAMES` в
`opt/sa02m-modbus-mqtt/bridge_mr02m_map.py`; копии в
`opt/sa02m-alice/sa02m_alice/config/inventory.py` (инвентарь пикера привязок) и
`www/network_config/static/js/mqtt.js` (`MR02M_TYPES`) сверяются тестом
`opt/sa02m-alice/tests/test_inventory.py`. Строки ниже — те же значения; кодов
13 и 14 не существует, а тип 15 (`4TO6DI`, триак-диммеры) отдаёт 4 канала и как
DO, и как AO.
...
**Цена на шине.** Модуль хотя бы с одним DI в режиме «Кнопка» платит
**3 транзакции FC04 за каждый проход** устройства в цикле (695 / 711 / 727,
каждая на `max_ch` регистров — до старшего канала-кнопки) плюс **1 FC03 на
`N_di` регистров раз в `poll_diag_s`** (60 с — перечитывание режимов 630+).
Для 6DO8DI это 3 → 6 транзакций за проход, то есть вдвое, на линии, которую
делят Carel (опрос 2 с) и MPLC4. Сбой блочного чтения счётчиков — событие
шины, не канала: per-channel `error=r` не ставится (как у DO/DI/AO), после
трёх сбоев подряд опрос счётчиков выключается до следующего перечитывания
режимов. Слияние трёх блоков в один FC04 (695..727+max_ch−1, ≤ 46 регистров)
отложено до стендового замера: обрыв длинного ответа на загруженной линии
уже зафиксирован на 42 регистрах (`bridge_mr02m_map.py`).
