...
**Тип:** Некорректное поведение через облачный прокси (принудительный выход по транзитному отказу)
**Описание:** Сколково, панель через `cloud.cyntron.ru/devcfg/<id>/`: «Применить» (и «Проверить» — с 1.0.6.49 это POST) в «Обновление веб» сразу выбрасывает на страницу входа; после повторного входа — то же самое.
**Причина:** Токен CSRF имеет один транспорт — заголовок запроса `X-SA02M-CSRF`, а прокси облака пересылал cookie, но пересылал только четыре именованных заголовка запроса и заголовок срезал. Плата отвечала `E_CSRF`, а обёртка `fetch` в `app.js` трактовала любой `E_CSRF` как потерю сессии (задумано для окна апгрейда 1.0.5.72: сессия старше cookie `sa02m_csrf`) — чистила cookie и редиректила на `login.html`. Транзитный отказ был неотличим от сессионного: тело `E_CSRF` не называло причину, а восстановить токен без повторного входа было нечем.
**Исправление:** `web_csrf_validate` оставляет причину отказа (`WEB_CSRF_FAIL_REASON`: `no_session|no_token_file|no_header|mismatch`), общее тело печатает `web_csrf_error_body` с полем `reason` (аддитивно; все 16 inline-сайтов переведены на него или добавляют поле). Новый GET-only `csrf_token.cgi` отдаёт токен только живой сессии (`no-store`, `nosniff`, доминт файла для сессии без него). Панель: `no_header` при отправленном заголовке → тост о прокси, сессия сохраняется, повтора нет; иначе — один `GET csrf_token.cgi` и один повтор запроса с новым заголовком; выход только при провале обновления или втором не-транзитном отказе; при загрузке страницы без зеркальной cookie токен запрашивается один раз (`init.js`). Требования к прокси — новый контракт `docs/contracts/cloud-panel-proxy.md` (облако пропускает семейство `X-SA02M-`). Гейты: `web-auth-behaviour` 30a–f, `cgi-csrf-behaviour` 11–14, новый `app-csrf-recovery` — RED-first на 91157d5 записан в заголовках харнессов. Решение: `docs/decisions/selective-csrf-policy.md` «Реакция панели».

## [2026-09-23 21:30] branch: 1.0.6.53

**Файл(ы):** `www/network_config/static/css/main.css` (блок ≤560 px), `.ai-dev/quality/checks/ui-layout.mjs`, `static/js/devices.js`
**Тип:** Вёрстка (телефон, вкладка «Управление»)
**Описание:** На телефоне (360–414 px CSS) карточки «Управление» стоят по две в ряд (~60/35 %), подписи и кнопки во второй колонке обрезаны («НЕ УСТАНОВЛЕНО», «Перезапустить», «Выбрать файл»). Облако предположило «CSS не догрузился без Referer» — нет: воспроизводится на голой панели с полным CSS.
**Причина:** `.system-manage-grid > .system-manage-services-card { grid-column: span 2 }` при auto-fill-сетке, которая на ≤ ~502 px резолвится в ОДИН трек: `span` не «зажимается до 1», а создаёт **неявную вторую колонку** (`grid-auto-columns: auto`), и в неё утекают все auto-размещённые плитки. Комментарий в CSS утверждал обратное. Гейт `ui-layout` не видел класс: 360 px — report-only, а обрезка ВНУТРИ карточки (не за краем viewport) не измерялась вовсе.
**Исправление:** В блоке ≤560 px `.dash-grid.system-manage-grid { grid-template-columns: minmax(0, 1fr) }` + карточка «Службы» `1 / -1`; комментарий исправлен. `ui-layout`: новые измерения (h) intra-box horizontal clipping (`overflow-x:hidden|clip`, `scrollWidth > clientWidth + 2`, кроме идиомы ellipsis) — гейт на поддерживаемых ширинах и на 360 px для «Управление»; (i) одна колонка ≤560 px (ширина каждой плитки = ширине сетки ± 0,5 px, floor 5 плиток). RED на 1.0.6.52: 52 нарушения в четырёх phone-portrait-ячейках «Управление» (10 из 11 плиток по 230/88 px в сетке 332 px; 3 тела карточек прячут 23–48 px); GREEN после правки; прямой прогон Playwright 360/375/393/414 × темы × варианты — одна колонка, ничего не обрезано. Попутно (F-B3): `devices.js` — восемь абсолютных `/api/devices…` заменены на относительные, новый гейт `no-absolute-api-paths` (RED: 9).

## [2026-09-23 21:30] branch: 1.0.6.53

**Файл(ы):** `www/network_config/cgi-bin/web_update_apply.cgi`, `static/js/app/status.js`, `docs/contracts/web-update.md`
**Тип:** Ложная ошибка (LAN, стенд 1.135, 2026-09-23 20:30)
**Описание:** «Применить» против локального git-сервера: панель сразу пишет «Ошибка обновления. См. Журнал событий.», хотя раннер жив; после F5 виден прогресс.
**Причина:** Вердикт CGI через 1 с после запуска — «легаси-лок ещё есть ИЛИ `kill -0 $!`». Быстрый клон даёт хелперу снять лок и сделать `exec` раннера меньше чем за секунду, а `kill -0` на дочернем `sudo`→root из-под `www-data` отвечает EPERM (bash читает как «мёртв»; та же ошибка в `_legacy_running` — ветка «уже выполняется» на плате не срабатывала никогда). Затем CGI записывал `error` в `update_status` — файл хелпера — и следующий GET воспроизводил ложь. Панель на `status:"error"` без кода сразу показывала ошибку, опрос не начинала.
**Исправление:** CGI судит по дому живости `lib_web_update.sh` (pid лока хелпера/раннера + `cmdline`, юниты) или по свежей транзакции на стадии «идёт»; `update_status` не пишет; `_legacy_running` — по `cmdline`, не по `kill -0`. Панель: на `status:"error"` без кода и на таймаут POST — один GET через 1,5 с; занято → опрос, иначе — ошибка (`_webUpdConfirmError`). Контракт: `docs/contracts/web-update.md` «POST — ответ после запуска». Гейты: `web-update-apply-guard` секция H (RED на 91157d5: H1, H1b, H2, H3b), `test-web-update-semver` раздел G (RED: 9). Фикстура прежних launch-случаев харнесса теперь держит лок хелпера, как настоящий хелпер (раньше `kill -0` на спящем шиме моделировал механизм, которого у платы не было).

## [2026-09-23 18:00] branch: 1.0.6.52

**Файл(ы):** `etc/sa02m-update-runner.sh`, `etc/systemd/sa02m-update-verify.service` (новый), `etc/sa02m-web-update-apply.sh`, `www/network_config/cgi-bin/web_update_apply.cgi`, `cgi-bin/lib_web_update.sh` (новый), `cgi-bin/reboot.cgi`, `static/js/app/status.js`, `static/js/app/services.js`, `static/js/i18n.js`, `scripts/03-webserver.sh`, `scripts/update-www-only.sh`, `scripts/pack-offline-update.py`
**Тип:** Полевой инцидент — OTA с панели «зависает» на 85 %, перезагрузка откатывает исправное обновление
**Описание:** Сколково, шесть плат с образа 1.0.6.49, OTA → 1.0.6.50 из панели. Прогресс-бар замер на «Проверка сервисов…» 85 % (страница после F5 уже показывала 1.0.6.50); после «Перезагрузки» плата ≈3–4 мин не отвечала и вернулась на 1.0.6.49. Ошибки в панели не было.
**Причина:** Три дефекта одной цепочки. (1) `sudo -n sa02m-web-update-apply` из CGI остаётся в cgroup `fcgiwrap.service` (в PAM-стеке sudo на плате нет `pam_systemd`; измерено на 1.135), а первый шаг health-gate после `stage=verifying` — `systemctl restart fcgiwrap` при `KillMode=mixed`: вся cgroup, включая раннер, получает SIGKILL — без EXIT-trap, `transaction.json` остаётся на `verifying/85`, imaging-lock и hold watchdog висят, nginx отдаёт новое дерево с диска. (2) `sa02m-update-recover.service` (`Before=nginx fcgiwrap`) на `verifying` с целым деревом вызывал весь путь restart+health: задания `restart fcgiwrap`/`reload nginx` на юниты, упорядоченные ПОСЛЕ recover, могут только выйти по таймауту, `units_active` находит nginx неактивным → откат целого дерева (замысел 1.0.6.8 «recover завершает обновление» при загрузке невыполним по построению). (3) GET-статус CGI отображал любую «идущую» стадию как `running` без теста живости — панель опрашивала вечно. Найдено попутно: одна проба `is-active` сразу после `restart` читала ещё-`activating` юнит как упавший (образец 1.135 от 2026-08-20 откатил исправное обновление); значение runtime-watchdog, снятое на окно apply, — shell-переменная, терявшаяся при `exec` (каждый успешный apply оставлял `RuntimeWatchdogUSec=0` до перезагрузки); `reboot.cgi` и второе «Применить» не смотрели на живую транзакцию.
**Исправление:** Раннер на входе `cmd_apply` читает `/proc/self/cgroup` и из `fcgiwrap.service` перезапускает себя transient-юнитом `sa02m-update-apply-<txn8>` (`systemd-run`, `KillMode=process`) — со СЛЕДУЮЩЕГО после доставляющего обновления: старый раннер `exec`'ит копию самого себя, так что доставляющее OTA на плате ≤ 1.0.6.51 по-прежнему замирает на 85 % и завершается при перезагрузке (recover → verify) или через `scripts/sa02m-update-remedy.sh`. Recover целое дерево не откатывает: ставит `boot_verify_pending` и запускает `sa02m-update-verify.service` (static, `After=nginx fcgiwrap sa02m-devices-api`) → `runner verify` (enable+tmpfiles + health-gate, без restart-наборов; откат только по реальному отказу с `E_HEALTH` + причиной); откат из recover не трогает nginx/fcgiwrap. Health-gate: два подряд `active` в окне 30 с, пропуск `ConditionResult=no`, выдержка `systemctl status` и причина в `error_message`. Значение watchdog хранится в транзакции (`runtime_wdt_prev_usec`), транзакция старого хелпера восстанавливает политику из `sa02m-watchdog.conf`. CGI: `runner_alive`/`stale` (120 с без живого раннера → `status:"error"`, `E_RUNNER_LOST`); панель останавливает опрос и называет следующий шаг (перезагрузка). `reboot.cgi` отказывает живому раннеру (`E_UPDATE_RUNNING`), хелпер — второму запуску поверх живой транзакции. Контракт: `docs/contracts/web-update.md` «Жизненный цикл apply»; гейты `update-cgroup-escape`, `update-recover-boot`, `update-conditional-restart` run 6, `web-update-apply-guard` G/R, `test-web-update-semver` F, `web-update-launcher-guard` — RED-first на 6ba943d записан в заголовках харнессов. Стендовое воспроизведение (RED на старом коде, GREEN на 1.0.6.52) — `docs/OFFLINE_UPDATE_HW_TEST.md` §3.5. RED-прогон на 1.135 добавил остаток, которого не было в плане: recover был убит собственным `TimeoutStartSec=300` посреди отката (перезапуски наборов при загрузке заняли 5 мин) — плата осталась на `rolling_back` с `/run/sa02m-imaging.lock` и `RuntimeWatchdogUSec=0` (аппаратный сторож выключен) до следующей загрузки; закрыто в том же выпуске (откат из recover без перезапусков, EXIT-trap возвращает сторож на SIGTERM, `runner reclaim` + `scripts/sa02m-update-remedy.sh` для платы без перезагрузки).
...
**Тип:** Устойчивость установщика к обрыву питания/сбросу
**Описание:** Офлайн-обновление 1.0.6.37 → 1.0.6.40 на 192.168.1.136 оборвалось аппаратным сбросом (журнал установщика обрывается на 22:14:43 «рваной» строкой, последний heartbeat обёртки 22:15:08, загрузка ≈22:20). Плата поднялась с `/etc/systemd/system/sa02m-flasher.service` размером 0 байт — systemd читает пустой юнит как **masked**, прошивальщик не работал; `bridge_led.py` версии 1.0.6.40 лежал поверх рваного пакета `/opt/sa02m-led` без `MB2WS_TEXT_BASE` — мост крэш-лупил (119 рестартов). Повторный запуск в 22:48 прошёл 13/13 PASS, при этом пост-проверки печатали PASS «не был запущен до обновления — состояние сохранено» про замаскированный юнит.
**Причина:** Сам сброс — аппаратный (два кандидата: HW-watchdog после стойла PID 1 и внешнее питание — 8D D4, разрешение вне кода). Разрушительным его сделали дефекты установщика: (1) `install -m` сначала обнуляет живой файл и только потом наполняет его, а writeback на ext4 `commit=600` отставал минутами — отсюда 0-байтовый юнит; (2) `05-mqtt.sh` копировал модули моста ДО общих пакетов, которые они импортируют, — потребитель оказался новее зависимости; (3) `sa02m_svc_capture` читал пустой файл как решение оператора («маска») и переустановку не делал; (4) пост-проверка обёртки считала снимок `svc-before` сильнее реального состояния юнита; (5) `install.sh` запускался в сессии SSH-транспорта, а гарантия PID1-watchdog была «холостой» — `systemctl set-property --runtime Manager RuntimeWatchdogSec=0 || true` никогда не проверялась.
**Исправление (1.0.6.41):** `sa02m_atomic_install` (tmp + fsync + rename-over) и кодмод по всем 131 живым `install -m` (юниты + `/usr/local/*`); общие пакеты Carel/LED — перед первым потребителем в `05-mqtt.sh` и `04-flasher.sh`, `sync` после каждого модуля; состояние `broken` в capture (пустой фрагмент ≠ маска оператора) и переустановка по дефолту первой установки; пост-проверка: замаскированный или пустой юнит — FAIL независимо от снимка; запуск `install.sh` через `setsid … </dev/null`; общий блок удержания runtime-watchdog с ЧТЕНИЕМ значения обратно (`busctl … RuntimeWatchdogUSec`), одним домом на установщик и OTA-runner. Гарантия: аппаратный сброс в любой момент оставляет живой файл старым или новым, но не пустым, а повторный запуск чинит, а не сохраняет поломку.
...
**Тип:** Уязвимость (межарендная утечка)
**Описание:** Образ, снятый с ПРИВЯЗАННОЙ платы, унёс бы `device.crt.pem` / `device.key.pem` / `pending_claim.json` и документ привязок донора на каждый клон. Шлюз опознаёт контроллер только по DN его mTLS-сертификата, поэтому две такие платы — ОДИН контроллер для `alice.cyntron.ru`: плата покупателя оказалась бы в чужом «Доме с Алисой» (чтение и команды), а `claim_token` из `pending_claim.json` позволил бы клону отвязать контроллер донора.
**Причина:** Тот же класс, что и у облачного агента (запись [2026-07-31 20:23]), но по Алисе очистка не была сделана вовсе: путь снятия образа никогда не трогал `/var/lib/sa02m-alice`. Первопричина — записанное обоснование DENY-строки в `docs/TZ_PRE_PRODUCTION_DONOR_CLEANUP.md` §4.3 («policy: preserve on factory reset»): к пути снятия образа применили политику factory reset, тогда как политики противоположные (образ обязан стереть идентичность, factory reset обязан её сохранить).
**Исправление:** Очистка на всех четырёх площадках облачного близнеца, шаг в шаг: `stream-after-cleanup.sh` (на доноре, ДО `dd` — ключ не входит ни в поток, ни в `raw.img.xz`, ни в рабочую копию PiShrink), `patch-firstboot-image.sh` (офлайн по смонтированному образу + БЕЗУСЛОВНАЯ фатальная проверка — покрывает снятие с `--no-id-reset` и старым stream-скриптом), `autorun.sh` / `autorun-fel.sh` (приёмники: единственная защита для образа, снятого ДО правки), плюс ручной `reset-alice-enrollment.sh`. Документ привязок приводится к пустому (uuid4-идентификаторы — это ПРИВЯЗКИ, перевыпускать их нельзя, донорские стендовые привязки не должны клонироваться вовсе), `client_enabled = false`, юнит снят с автозапуска. `ca.crt.pem` (общий CA шлюза), `server.conf` и остальные ключи `client.conf` — конфигурация, сохраняются: пере-очистка тихо сломала бы клиента на всех клонах. `cleanup-donor.sh` намеренно НЕ трогали — его DENY охраняет glob-сборщик мусора; ослабление списка безопасности ради одного файла открыло бы дерево любому будущему глобу. Factory reset не изменён. Контракт: `docs/contracts/image-identity-reset.md`; гейт `alice-image-identity` (RED на 17 мутациях).
**Второй слой того же дефекта (ревью, F1):** списки из литеральных имён пропускали **sidecar'ы атомарной записи** — `api.py` пишет `<путь>.tmp` и лишь потом делает `os.replace`, поэтому сбой посреди привязки оставляет `device.key.pem.tmp`: тот же приватный ключ под именем на один символ длиннее, а фатальная проверка по трём именам такой образ пропускала. Закрыто по форме, а не по именам: `/var/lib/sa02m-alice/*.tmp` и `/etc/sa02m-alice/.alice-*` (форма облачного близнеца `frpc.toml.bak*`, потерянная при переносе), а сама проверка переписана на **белый список** — в каталоге идентичности имеет право остаться только `ca.crt.pem`, поэтому sidecar любой будущей формы тоже роняет сборку. Тем же ревью (F2) закрыт пробел в гейте: он проверял только ОПРЕДЕЛЕНИЕ функции очистки, и удаление её ВЫЗОВА на четырёх площадках из пяти оставляло гейт зелёным — добавлены пины на вызов.
**Проверено:** утечка НЕ материализовалась в отгруженных артефактах — аудит 2026-08-27 (loop-mount `tools/imaging/stand-data/images/sa02m-1eth-golden-20260820.img.xz`) нашёл только публичный `ca.crt.pem`, без `device.key.pem` / `device.crt.pem` / `pending_claim.json`, облачная половина корректно сброшена (`enrolled = false`). Но в том же образе найдены тестовая привязка (`id: d1`, «Lab Switch») и `client_enabled = true` — то есть половина списка очистки уже уезжала в образ. Итог: «починить до следующего снятия образа», без отзыва артефактов.
...
**Тип:** Некорректное поведение
**Описание:** Любая прошивка MR-02m через web-флешер на стенде .135 обрывалась на последних ~4 блоках (596–598/600 у обычного образа, 621/701 у BACnet-сборки), модуль оставался в загрузчике. Три повторных попытки падали в той же зоне.
**Причина:** Web-флешер слал каждый блок в легаси-окно `DATA_BLOCK_REG = 0x2000`. В этом окне загрузчик продвигает `flash_offset` на КАЖДЫЙ принятый кадр — включая повтор блока после потерянного ACK; повтор сдвигает образ, сдвиг накапливается, финальные блоки уходят за регион → исключение Modbus. Класс бага задокументирован в MR-02m (BUGLOG 2026-07-09/10) и давно исправлен в standalone-флешере переходом на индексированные окна BL ≥ 0.0.0.11: `0x2100+idx` (приложение) и `0x2600+idx` (BL-staging), где дубль с `idx <` ожидаемого получает идемпотентный ACK без записи. **Ошибочная промежуточная гипотеза (отозвана):** «BACnet-сборка (701 блок) не помещается в app-регион» — опровергнута firmware-командой (payload 172 320 Б при разделе 192 512 Б, запас ~20 КБ; эталонная прошивка того же образа на чистой линии — 50,3 с без ошибок). Падение на 621 было тем же накоплением сдвига.
**Исправление:** Индексированные окна портированы в `flash_protocol.py` (1.0.5.65, коммит `79a06c1`): флаги `use_indexed_data_blocks` / `use_indexed_bl_blocks` по версии BL (≥ 0.0.0.11 / ≥ 0.0.0.21, легаси `0x2000` — запасной путь для старых/неизвестных BL), маршрутизация на всех четырёх путях отправки, `block_index` проброшен в BL-staging. 19 юнит-тестов (регистр-по-индексу, матрица версия-гейта, non-overlap assert), суите 190 зелёных, Reviewer APPROVED (регистры сверены с `bootloader_config.h`). **Проверка на железе (.135):** застрявший модуль addr 10 восстановлен прошивкой 600/600 (та же падала 3×); затем BACnet-образ 701/701 лёг полностью, и пройден полный on-wire цикл: `122=2` → перезагрузка в BACnet → пассивный сниф `alive=True` (200 кадров MS/TP @38400) → ring-join WriteProperty MSV:1 (MAC 11→10) → модуль вернулся на Modbus. Риск «Python-мастер не войдёт в кольцо на A40i» опровергнут на 2-узловом кольце.

---

## [2026-08-05] branch: feature/klogic — плата не грузится после прошивки: генератор boot.scr выпускал невалидный U-Boot script image

**Файл(ы):** `tools/imaging/patch-firstboot-image.sh`, `tools/imaging/make-image.sh`, `tools/imaging/uboot-script.py` (новый), `tools/debian-rootfs/pack-sa02m-image.sh`, `etc/boot.cmd.sa02m.min` (новый), `etc/boot.cmd.sa02m`, `.gitattributes`, `scripts/dev/test-uboot-bootscr.sh` (новый), `docs/contracts/uboot-boot-script.md` (новый)
**Тип:** Некорректное поведение
**Описание:** Плата, прошитая образом из пайплайна, останавливалась в U-Boot: ядро не стартовало, пищалки и сети не было, восстановление только через FEL. Тот же класс отказа уже случался 2026-08-03 (запись «MPLC4 image boot.scr restored from donor», `boot.scr` 1960 байт вместо валидных 261).
**Причина:** **Корневая причина — ГЕНЕРАТОР, а не артефакт.** 2026-08-03 починили только артефакт (скопировали `boot.scr` с донора `.136`), а `build_bootscr()` остался сломанным — отсюда повтор. В запасном Python-пути (`patch-firstboot-image.sh:85`) писалось `header + data`, тогда как для `ih_type=6` (`IH_TYPE_SCRIPT`) U-Boot читает данные как multi-file image: сначала 8-байтовая таблица размеров `[uint32 BE len][uint32 BE 0]`, затем текст. Без неё за длину принимаются первые четыре байта скрипта (`"# SA"`). Арифметика донора закрывает вопрос: 261 = 64 + 8 + 189, padding отсутствует. Ещё три дефекта на том же пути: `int(time.time())` вызывался дважды (⇒ «Bad Header Checksum»); CRLF не снимался и CR уходил в `setenv bootargs`; `make-image.sh:284` глушил падение как `WARN … (не критично)`, а сам шаг был обёрнут в `[ -x … ]` при режиме файла 100644 — на свежем клоне управление молча уходило в ветку-заглушку, которая патчила watchdog-юниты, но `boot.scr` не собирала вовсе. Единственной проверкой был `grep -aq 'threadirqs'`, который невалидный файл проходит.
**Исправление:** Формат вынесен в единственный дом `tools/imaging/uboot-script.py` (build + validate + выбор payload), которым пользуются оба производителя и тест — writer и validator больше не разойдутся. Writer пишет таблицу размеров, `ih_size = 8 + len(script)`, `ih_dcrc = crc32(table + script)`, без padding, timestamp снимается один раз. Побайтовый validator (10 утверждений) применяется к выходу обоих путей, включая `mkimage`, и заменил `grep`. `mkimage` стал жёстким требованием шага 0; патч-шаг стал безусловным, вызывается через `bash` (режим файла больше не решает, применён ли патч), заглушка `patch_image()` удалена, падение фатально (`die` вместо `WARN`). Payload по умолчанию — проверенный на плате 189-байтовый `etc/boot.cmd.sa02m.min`; `etc/boot.cmd.sa02m` оставлен как opt-in (аргумент `--bootcmd <path>`, а не переменная окружения: `sudo` с `env_reset` срезал бы её) с маркером `UNVERIFIED-ON-HARDWARE`; оба payload'а пришпилены к LF. `pack-sa02m-image.sh` переведён на тот же селектор и validator. Перед записью проверяется наличие `zImage` и `sun8i-a40i-sk.dtb` на FAT p1 (у минимального скрипта нет fallback по именам DTB). Формат зафиксирован тестом `scripts/dev/test-uboot-bootscr.sh` (quality row `uboot-bootscr-format`): golden привязан к фактам донора, пять невалидных форм отвергаются, тест падает на ПРЕ-фикс генераторе (253 ≠ 261). Контракт: `docs/contracts/uboot-boot-script.md`.
...
```
systemd[1]: Failed to set timeout to 2min: Invalid argument
systemd-shutdown[1]: Failed to set timeout to 2min: Invalid argument
```
Предел самого драйвера при этом печатается в dmesg:
```
sunxi-wdt 1c20c90.watchdog: Watchdog enabled (timeout=16 sec, nowayout=0)
```
**Причина (ПОДТВЕРЖДЕНО):** `RebootWatchdogSec=2min` (= 120 c) в `etc/systemd/sa02m-watchdog.conf` при hardware cap sun4i-wdt (Allwinner A40i) **16 c**. Systemd на плате (Debian bullseye) запрос выше кэпа не клампит, а отвергает: fallback «переиспользовать уже запрограммированный таймаут» у него есть только для ошибки «не поддерживается», а для EINVAL — нет, ретрая тоже нет. Поэтому значение просто никогда не применялось.

Вторая, более важная находка того же разбора: **штатный установщик вообще не использовал файл политики.** `install.sh` → `scripts/01-system.sh` писал heredoc'ом СВОИ значения (`RuntimeWatchdogSec=10s`, `ShutdownWatchdogSec=4min`, `RebootWatchdogSec=4min`) в тот же самый путь `/etc/systemd/system.conf.d/sa02m-watchdog.conf`. Это не fallback, а конкурирующий писатель: на устройствах, поставленных или обновлённых штатным установщиком, стояло 10s/4min/4min, а `etc/systemd/sa02m-watchdog.conf` для этого пути был декоративным. Всего копий политики в репозитории нашлось девять.

**НЕ ПОДТВЕРЖДЕНО (гипотеза, и оставлена гипотезой):** цепочка «медленное выключение → срабатывание взведённого на 16 c watchdog-а → hardware reset посреди записи → повреждение ext4 → плата грузится после прошивки, но не после перезагрузки». Она правдоподобна и согласуется с полевым симптомом, а бюджет таймаутов делает её достижимой (`DefaultTimeoutStopSec=15s`; у mplc4/klogic/nodered/sa02m-flasher свои `TimeoutStopUSec=15s`; плюс финальный sync рутфс на ~7 ГБ — 16 c выбираются легко). Но есть контр-факт: **чистая тёплая перезагрузка на стендовой плате заняла ~1 c и прошла без повреждений.** Связь watchdog-а с полевыми повреждениями ФС НЕ доказана, и данное исправление её не «чинит»: оно устраняет подтверждённую ошибку конфигурации (EINVAL) и снимает *возможный* аппаратный сброс во время выключения.

**Исправление:**
1. Политика приведена к `RuntimeWatchdogSec=15s` + `RebootWatchdogSec=0`; `ShutdownWatchdogSec` (устаревший, до systemd v243, псевдоним `RebootWatchdogSec`) удалён везде — две ручки на одну политику и были механизмом расхождения. `0` — документированное «выключено» по `systemd-system.conf(5)` (устройство не открывается, не настраивается и не пингуется). Отключение watchdog-а на время выключения выбрано осознанно: любое допустимое значение (≤16 c) ребутило бы нормальное, но медленное выключение, а целостность ФС дороже автоматического выхода из зависшего shutdown. Для железа это безопасно: закрывая `/dev/watchdog`, PID1 шлёт `WDIOS_DISABLECARD` и magic-close `V` (источник: systemd, `src/shared/watchdog.c`, `watchdog_close()`), а драйвер сообщает `nowayout=0` (из dmesg выше) — значит magic close честен и watchdog разоружается, а не остаётся взведённым. Прочитано по исходнику systemd, на плате не перепроверено (см. критерий ниже). Обоснование и принятый компромисс записаны в шапке `etc/systemd/sa02m-watchdog.conf` (единственный дом политики).
2. `scripts/01-system.sh`: heredoc заменён на `install -m 644 "$ETC_REPO/systemd/sa02m-watchdog.conf" …`. Отсутствие эталона теперь громкая ошибка (`log ERR` + `exit 1`), а не тихий fallback — неполный checkout лучше уронить, чем поставить другую политику. Заодно исправлен комментарий, утверждавший «таймаут драйвера sunxi-wdt (~30s)»: там 16 c.
3. Шесть копий, где репо недоступно по построению (stdin-режим `tools/system-hardening/install.sh`, imaging с USB-носителя), оставлены fallback-ами, но приведены к тем же значениям: `firstboot-overlay` (побайтовая копия эталона), `patch-firstboot-image.sh`, `autorun.sh`, `autorun-fel.sh`, `tools/system-hardening/install.sh`. Плюс `ssh-flash-safe.sh` (8s → 15s) — единственная копия, правящая `/etc/systemd/system.conf` напрямую, а не drop-in; помечена комментарием, перевод на drop-in вынесен в backlog.
4. Новый гейт `.ai-dev/quality/checks/watchdog-cap.sh` (строка реестра `watchdog-cap`, beat `build`): нормализует длительности (`2min` → 120 c — наивное числовое сравнение прочитало бы «2» и пропустило бы ровно эту ошибку), требует ≤16 c или явного disable, пиннит значения во всех копиях, запрещает активный `ShutdownWatchdogSec=`, сверяет overlay с эталоном побайтово (`cmp`) и держит `01-system.sh` на установке из репо. Явно НЕ флагует imaging-овые `=0` и закомментированные строки — иначе гейт бы отключили.
5. Диагностика: `tools/system-hardening/install.sh` рядом с `systemctl show -p RuntimeWatchdogUSec …` печатает потолок драйвера `/sys/class/watchdog/watchdog0/max_timeout`. Именно `max_timeout`: соседний `timeout` — это ТЕКУЩИЙ запрограммированный таймаут (после этой политики там 15), максимумом он не является.

**Проверка на плате (ещё НЕ выполнена; фальсифицируемый критерий):** после деплоя по рецепту `tools/system-hardening/README.md` — `systemctl show -p RebootWatchdogUSec` → `0`, затем `reboot` и `journalctl -b -1 | grep -i "Failed to set timeout"` → пусто. Непустой вывод = исправление не сработало. Отсутствие повреждений ФС этот сценарий не проверяет и проверить не может (см. «НЕ ПОДТВЕРЖДЕНО» выше).

**Почему гейт, а не просто очередной фикс значения:** это ТРЕТИЙ случай одного класса. `[2026-07-06 15:15] branch: 1.0.3.37` — `RuntimeWatchdogSec=25s`, тот же EINVAL, починили значение. `[2026-06-02 12:30] branch: 1.0.3.22` — `RuntimeWatchdogSec=10s` при zero-fill eMMC ребутил донора во время снятия образа. Каждый раз правили число и ни разу — отсутствие механической проверки.
