...
(замороженный v1: только обязательные ключи) — иначе плата на
документированном полу 1.0.5.66 отвергает пакет с `E_MANIFEST`.
`sa02m-rules` остаётся в обязательном `restart[]` (старый runner его
исполняет). Условные рестарты Alice/моста и `enable[]` пакер начнёт писать
после поднятия `MIN_UPDATER` до валидатора, который эти ключи принимает.
