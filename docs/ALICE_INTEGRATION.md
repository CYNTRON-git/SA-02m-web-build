...
клиента, привязка/отвязка, добавление устройств по MQTT-топику
(переключатели и датчики).

## Датчики

В окне «Управление привязками» поле «Тип» позволяет привязать реальное
значение как датчик Алисы: Температура и Влажность (ДТВ — климат-датчик),
Напряжение, Ток и Мощность (СЭ-02м-3 — датчик). Одна привязка = одно
устройство Алисы (например, ДТВ с температурой и влажностью — два
устройства). Топики ДТВ и СЭ появляются в списке автоматически из
конфигурации моста Modbus→MQTT.

- Значения передаются в инженерных единицах (°C, %, В, А, Вт) без
  пересчёта.
