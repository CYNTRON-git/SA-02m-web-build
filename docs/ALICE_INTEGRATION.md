...
**Сцена как выключатель (с 1.0.6.41).** Сценарий, помеченный «в Алису» в
облачном редакторе (включённая строка `type: scene` с `alice_expose`),
появляется в «Доме с Алисой» отдельным выключателем с именем сцены и в
комнате, которую называет `captured_from.room_id`: «Алиса, включи <сцена>»
запускает её, «выключи» — гасит те выходы `on_off`, которые сцена включает.
Состояние выключатель не сообщает (плата не знает, «включена» ли сцена
сейчас), поэтому Алиса рисует две кнопки, а не тумблер. Список у пользователя
обновляется его же «Обновить список устройств» — плата ничего не
перезапускает. На карточке «Умный дом» такие сцены видны отдельными
**строками только для чтения** со значком «сценарий» и счётчиком «Сценариев
в Алисе: N»: помечают и правят сцены в облачном редакторе, не в панели.
Форма устройства, id, комната и жизненный цикл —
`docs/contracts/alice-mqtt-mapping.md` §Scene devices; что при этом исполняет
движок — `docs/contracts/cloud-scenarios.md` §MQTT mirror.
...
недоступна, пока плата не привязана к облаку. До 1.0.6.29 кнопка стояла на
карточке «Умный дом»; она переехала туда, где уже живут привязка к облаку и её
состояние.
