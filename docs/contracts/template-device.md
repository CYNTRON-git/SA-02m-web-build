...
Оба теста бегут под строкой реестра `py-unit`, которая с 1.0.6.40 перечисляет
этот файл в `covers`. Честный предел (аудит 2026-09-08 F17): `mqtt_templates.cgi`
проверяется только синтаксисом (`bash-cgi-syntax`) — поведенческого теста
эндпойнта нет; закрыл бы его bash-харнесс в духе `web-auth-behaviour`.
