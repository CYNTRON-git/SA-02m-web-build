...
**Этот раздел — дом словаря stand-down для ОБЕИХ дверей.** С 1.0.6.32 сброс
привязки написан один раз (`opt/sa02m-cloud-agent/binding_core.py`, побайтовая
копия в пакете умного дома; строка реестра `binding-reset-parity`), и клиент
Алисы пишет в свой статус-файл те же `reason` / `reason_class` / `unlinked_at` /
`restored` с тем же смыслом. Перечень состояний у дверей РАЗНЫЙ и это верно: у
двери Алисы нет оттенка `revoked` (событие шлюза не отличает отзыв владельца от
отвязки), поэтому её перечень — свой, в `docs/contracts/alice-mqtt-mapping.md`
§Client status file; сюда он только ссылается за формой ключей. Ничего нового
здесь не появилось: ни состояния, ни ключа.
...
Со стороны рендера тот же перечень проверяет `scripts/dev/cloud-card-smoke.mjs`
(строка реестра `cloud-card-smoke`, review beat): каждое состояние строки
`state ∈ …` рисуется в headless Chromium, и состояние без фикстуры — провал.
