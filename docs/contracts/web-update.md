...
| Остаток без перезагрузки | `runner reclaim` (через `scripts/sa02m-update-remedy.sh`) | Тот же recover во время работы для транзакции, чей раннер исчез: при живом раннере (лок занят) — ничего не делает; `rolling_back`/`applying` — откат дозавершается с перезапусками (веб поднят); целое `verifying`/`committing` — уходит в `sa02m-update-verify.service`, который запускается сразу; терминальная стадия с оставшимся локом — лок снимается, сторож восстанавливается. Рецепт и ожидаемый вывод — `docs/deployment.md` «Ремонт застрявшего обновления без перезагрузки». Что не делается само: без перезагрузки и без вызова `reclaim`/нового apply остаток лежит — периодического вызова нет (решение о таймере — отдельное) | `update-recover-boot` R8, M |
