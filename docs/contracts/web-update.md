...
| Раскладка файла (с 1.0.6.54) | раннер, `journal_append` → `atomic_install_file` | **G6.** Строка журнала, описывающая файл (`op`, `dst`, `backup`, `mode`, `owner`), лежит на диске (`fdatasync`) **до** переименования, которое она описывает; порядок на каждый изменённый файл: `fdatasync(backup)` → `fdatasync(journal)` → `fdatasync(tmp)` → `rename` → `fsync(dir)`. Копия для отката лежит на диске **до** строки журнала, которая её называет: корневая ФС платы — `commit=600` + `journal_data_writeback`, и `fdatasync` журнала фиксирует inode копии, но не её данные — без собственного `fdatasync` копии откат после обрыва питания ставил бы пустой файл поверх живого (с 1.0.6.54, ревью R4-1). Обрыв питания в любой точке оставляет старый или новый файл (никогда усечённый), журнал, который его уже знает, и копию, из которой его можно восстановить, — откат по журналу не оставит НОВЫЙ файл, о котором журнал молчит. Каждый шаг проверяется, и провал любого из них — отказ всего apply (`E_APPLY`, откат) **до** переименования, а не «файл засчитан»: копия для отката (`backup`) и её `fdatasync`, запись строки журнала, её `fdatasync`, `install`, `fdatasync(tmp)`, `mv`, `fsync(dir)` — без отката на полный `sync`, который об ошибке сообщить не может (провал `fsync(dir)` случается уже после переименования; строка журнала файл уже описывает, откат его восстанавливает). То же для удаления по манифесту (`delete[]`: копия → `fdatasync` копии → строка журнала → удаление) и для отметки версии раннера. Если не прочитан список `stop_before_apply`, apply останавливается (`E_APPLY`) **до** раскладки — `sa02m-flasher` не остаётся держать порты RS-485 во время обновления. Файл в `/etc/sudoers.d/` проверяется `visudo -cf` **до** переименования; отказ — `E_APPLY`, живое правило не тронуто (одна синтаксическая ошибка там ломает `sudo` целиком, а с ним и сам запуск обновления). **Откат по журналу** восстанавливает каждую запись отдельно и атомарно (временный файл рядом → владелец и права копии → `fsync` → переименование), пропускает оборванную последнюю строку; если какую-то запись восстановить не удалось, остальные всё равно восстанавливаются, а транзакция получает `stage=error` с `error_message` «rollback incomplete (…)», а не `rolled_back` | `update-deploy-skip` случаи 6 (порядок, от `fdatasync` копии), 7 (экранирование `dst`), 9a/9b (отказ install/mv), 10c (список раскладки прочитан не до конца), 11a–11d, 11f (отказ копии / записи журнала / его fdatasync / журнала отметки / имени копии), 11g/11h/12e (отказ `fdatasync` копии — раскладка / отметка раннера / удаление), 12a–12d (удаление), 13a/13b (отказ fdatasync(tmp) / fsync(dir)), 14a/14b (`stop_before_apply`), 15a–15e (откат: оборванная строка, хвост из нулей, нет копии, сбой одной записи, атомарность и владелец), 16a/16b (`visudo`) |

Сбой чтения манифеста там, где раннер работает с `errexit` (версия и коммит
цели сразу после распаковки пакета — стадия `validating`; список файлов для
архива отката — стадия `backing_up`; `commit_markers` — стадия `committing`),
не маскируется, а **обрывает раннер** с ненулевым кодом: `on_exit` возвращает
сторож, транзакция остаётся на своей стадии, панель через 120 с показывает
`stale`, `recover` при загрузке доводит её — `validating` и `backing_up` (архив
ещё не записан в транзакцию) заканчиваются `E_POWER` без изменения файлов,
целое дерево на `committing` уходит в `sa02m-update-verify`. Это поведение
задумано, а не случайно (с 1.0.6.54).
