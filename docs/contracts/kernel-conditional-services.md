...
- **MPLC4 — обратная политика: включён по умолчанию при первой установке**
  (`scripts/09-mplc.sh`: первая установка ⇒ enable+start — штатный рантайм
  изделия; обновление ⇒ точное восстановление прежнего состояния оператора,
  `docs/contracts/installer-refresh-policy.md`). Это осознанно и НЕ должно
...
## 6. Линия ядра флота и дефолты переключателя SMP/RT

Флот несёт ровно два ядра, между которыми переключает веб-панель:

| Профиль | `uname -r` | Каталог модулей |
|---|---|---|
| SMP | `6.1.0-rc6` | `/lib/modules/6.1.0-rc6` |
| RT (`PREEMPT_RT`) | `6.1.0-rc6-rt4` | `/lib/modules/6.1.0-rc6-rt4` |

Производитель — `tools/buildroot/prepare-rt-docker-kernel.sh` (единственный путь
сборки ядра в репозитории); оба профиля подтверждены на стенде в 1.0.5.58 (§4).
Контекст решения: `.ai-dev/notes/kernel-line.md`.

**Гарантия (пин 8).** `etc/sa02m-kernel-select.sh` держит эту пару в двух
местах, и оба обязаны с ней совпадать:

- `SMP_VER_DEFAULT` / `RT_VER_DEFAULT` — дефолты, которые действуют, пока нет
  `/etc/sa02m_kernel.conf`;
- `case`-шаблон в `detect_installed_module_ver()` — белый список имён каталогов
  `/lib/modules/`, которые скрипт вообще согласен считать нашим ядром. Шаблон
  обязан матчить **оба** дефолта.

**Почему это гейт, а не соглашение.** Дефолт, который не совпадает ни с одним
каталогом `/lib/modules/` и который детектор не умеет найти, не остаётся
безобидным: `load_conf()` не чинит его, а `write_conf()` записывает его в
`/etc/sa02m_kernel.conf` при первом же вызове. Дальше `cmd_set` для этого
профиля отвечает `modules_missing`, и панель отказывается переключиться на
ядро, которое на плате физически есть. Ровно этот отказ жил в скрипте с
дефолтами 5.10.35/5.10.35-rt36 на 6.1-плате без conf: шаблон детектора
(`*sa02m*|5.10.35*`) не матчил ни `6.1.0-rc6`, ни `6.1.0-rc6-rt4`.

**Смена линии ядра** = правка этой таблицы + дефолтов + шаблона детектора одним
изменением; гейт не даст разъехаться. Исторические имена (`*sa02m*`, `5.10.35*`)
в шаблоне оставлены сознательно — ради плат, которые могли не мигрировать; список
намеренно узкий, чтобы детектор не подхватил стоковое ядро дистрибутива.
