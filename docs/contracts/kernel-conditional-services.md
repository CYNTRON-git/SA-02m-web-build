...
- **MPLC4 — обратная политика: включён по умолчанию** (`scripts/09-mplc.sh`
  делает `systemctl enable mplc4` + `restart`). Это осознанно и НЕ должно
  приводиться к CODESYS-модели «только runtime» (решение Оператора 2026-08-07):
  MPLC4 — штатный рантайм изделия и стартует сам. Отдельный образ, где рантайм
  на изделии стоит `disabled` до активации лицензии в панели, — это выбор
  конкретного образа, а не политика установщика; `enable`-по-умолчанию остаётся.
