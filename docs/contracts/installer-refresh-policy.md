...
5. **Установка на чистую плату** (режим по умолчанию — `full`): те же
   модули. Дефолты первой установки прикладных юнитов задаёт каждый модуль
   (`app on|off`): семейство Алисы и **Docker** — `app off` (заявлено с
   1.0.6.37, **фактически с 1.0.6.39**: до этого `12-docker.sh` снимал
   состояние ПОСЛЕ установки пакета, postinst `docker.io` уже включил и
   запустил юнит, и «первой установки» apply не видел — автозапуск
   сохранялся; порядок теперь приколочен `installer-svc-helpers` случай 13);
   refresh это never-widen, уже включённый оператором Docker остаётся
   включённым.
...
- Свидетели против ложного «нового юнита»: `absent` требует трёх подтверждений
  (`is-enabled` отвечает «юнит неизвестен», нет unit-файла на диске, менеджер
  отвечает на `is-active`). «Юнит неизвестен» имеет две формы: systemd <250 —
  rc=1 с пустым выводом, systemd ≥250 — вывод `not-found` и rc=4 (замер на
  стенде 1.135, systemd 255, 23.09.2026); обе читаются как `absent`, иначе
  первая установка `app`-юнита не распознаётся, а `infra`-юнит без файла даёт
  WARN на каждом refresh. Зависший D-Bus читается как `timeout` — ничего не
...
- Состояние `broken` (с 1.0.6.41): `is-enabled` говорит `masked`, но фрагмент
  юнита в `/etc/systemd/system/<unit>` — **обычный файл нулевого размера**.
  Маску systemd делает симлинком на `/dev/null`, поэтому пустой файл там может
  быть только обрывом прошлой установки (стенд 1.136, 08.09.2026:
  `sa02m-flasher.service` 0 байт после аппаратного сброса посреди установки).
  Модуль уже переустановил файл, поэтому `app`-юнит применяет дефолт ПЕРВОЙ
  установки с WARN, а `infra`-юнит включается без `unmask` (снимать нечего).
  Правило «маску оператора не трогаем» этим не ослаблено: симлинк на
  `/dev/null` и непустой файл, о котором менеджер говорит `masked`, остаются
  `masked` и сохраняются.
...
- Программный сторож ставится и запускается любой установкой, включая refresh
  (с 1.0.6.51; раньше — только образом): `infra`-юнит, скрипт и unit пишутся
  атомарно, `/etc/sa02m_userspace_watchdog.conf` — только если его нет. Установку
  защищает только блокировка выше (льгота сторожа 180 с считается от загрузки).
  Следствие, принятое Оператором: установка, после которой nginx или страница
  входа не поднялись, даёт цикл перезагрузок примерно раз в 13 мин; описание и
  выход — `docs/deployment.md`, «Программный сторож после установки».
