...
rollback-архивы в `/var/lib/sa02m-update/rollback/` **удаляются** как часть очистки незавершённого обновления: «золотой» образ не должен нести чужую историю обновлений (запечённая транзакция + её резервная копия ломают `sa02m-update-recover` при первой загрузке прошитой платы). Учтённые метки версии (`state/deployed_*`) и доверенные ключи при этом сохраняются (их убирает только `--purge-update-state`).
...
> **Почему `/var/lib/sa02m-alice` в DENY.** Не потому, что идентичность нужно
> сохранить — при снятии образа её как раз обязаны стереть (**снятие образа и
> factory reset — противоположные политики**: образ стирает идентичность,
> factory reset её сохраняет; смешение этих двух политик и было причиной того,
> что привязка Алисы уезжала в каждый клон). Причина DENY другая: этот скрипт —
> glob-сборщик мусора, и ослабление его списка безопасности ради одного файла
> открыло бы всё дерево любому будущему глобу. Идентичность снимает отдельная
> именованная стадия `stream-after-cleanup`.
