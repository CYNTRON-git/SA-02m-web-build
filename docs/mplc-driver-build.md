...
6. **Проверить, что набор каналов НЕ сузился** — сравнивать с предыдущим
   бинарём, а не глазом по одному префиксу. Проверка НЕ ДОЛЖНА проходить
   вхолостую: пустой список с обеих сторон означает, что `strings` недоступен
   или регулярка промахнулась, а не «регрессий нет».
   ```bash
   chan() { strings -n 4 "$1" | grep -xE      '(CPU|RAM|DISK|SD_CARD|USB_FLASH|MODEM)_[A-Z_]+|UPTIME|SYS_TIME|RTC_TIME|BLUE_LED|RED_LED|BUZZER|DOUT|USB_POWER'      | sort -u; }
   chan old.so > /tmp/a; chan new.so > /tmp/b
   [ -s /tmp/a ] || { echo 'ВХОЛОСТУЮ: старый бинарь дал пустой список'; exit 1; }
   diff /tmp/a /tmp/b    # ничего не должно ИСЧЕЗНУТЬ (строки с '<')
   ```
   Эталон боевого билда 2026-08-21 — 22 канала.
...
## 8. Боевой `mplc_cyntron.so` в этом репозитории (2026-08-21)
...
- **публикация лицензии рантайма** (с 1.0.6.5) — `InitAddin` запускает
  отсоединённый поток с ограниченным ожиданием, который дожидается загрузки
  ключа защиты и один раз атомарно пишет `/run/sa02m-mplc-license.json`
  (режим 0644). Ожидание нужно потому, что рантайм инициализирует защиту ПОСЛЕ
  загрузки аддонов (замер на стенде: аддоны T+0.4 с, блок `<Protect>` T+0.8 с).
  Читает штатными экспортами `masterplc.so` — `GetFeatureParameter` и
  `GetProtectInfo`; идентификаторы `FeatureParameter` НЕ дублируются здесь, их
  единственный дом — `docs/agent-rules/mplc4-api.md` (это соответствие уже один
  раз разъехалось в продакшене — точки читались из `InstancesLimit`).
  Плата без ключа получает
  `{"activated":false,"key_error":N}`, а НЕ демонстрационные лимиты рантайма
  (иначе панель показала бы безлимитную лицензию). Потребитель и порядок
  источников: `docs/contracts/mplc-project-deploy.md`.
...
**При замене драйвера пересобирай ВСЕ слои (правильный SDK + проектные правки +
публикацию лицензии); голый вендорный исходник теряет функционал.** Текущий
отпечаток боевого билда — `f6ae6026`, 186356 B (2026-08-21); он же обязан
стоять на устройстве, см. приёмку в `docs/deployment.md`.
