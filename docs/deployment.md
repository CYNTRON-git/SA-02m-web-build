...
**проверяет сумму прямо на носителе** (чтение в обход кеша страниц). Признаки в
`/var/log/sa02m-rootfs-expand.log`, ≈T+30 с после старта:

- `primary superblock checksum OK …` — гарантия выполнена; питание снимать
  можно в любой момент.
- `ERROR: primary superblock checksum still not OK …` + файл
  `/var/lib/sa02m-rootfs-expand.csum-bad` + юнит `sa02m-rootfs-expand` в
  состоянии `failed` — гарантия НЕ выполнена. Плату **нельзя** обесточивать до
  одной чистой перезагрузки (`systemctl reboot`); после неё маркер остаётся как
  сигнал для аудита — снять вручную и разобраться, почему freeze не помог.
...
дождаться `checksum OK` в логе, снять питание на +2 мин, холодный старт —
плата обязана подняться.
...
3. Инструмент делает дампы «до», два прохода `e2fsck -fy` (первый только
   ...
   systemd и маркеры первого старта в `repair-N/` на носителе.
