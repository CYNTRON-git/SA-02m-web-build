...
**не пишет** эти ключи, пока `MIN_UPDATER` < 1.0.6.37, иначе плата на
документированном полу 1.0.5.66 отвергает пакет с `E_MANIFEST`. Онлайн-OTA
...
после 1.0.6.37 обновления; оффлайн-пакет 1.0.6.37 с замороженным v1 принимается
платой ≥ 1.0.5.60 / runner ≥ 1.0.5.66 (см. `docs/OFFLINE_UPDATE_PACKAGE_V1.md`).
...
`docs/contracts/bench-board-target-state.md`.
