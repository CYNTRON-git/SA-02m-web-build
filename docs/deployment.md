...
После обновления файлов `/opt/sa02m-*` перезапускать весь набор читающих их
служб: `sa02m-rules`, `sa02m-alice-client`, `sa02m-modbus-mqtt` (если менялся
мост) **и `sa02m-cloud-control`** — облачный профиль держит в памяти те же
модули (`sa02m_alice` + `sa02m_rules.store` из `/opt/sa02m-rules`), и без его
рестарта сценарный канал молча работает на старом коде (приёмка 1.0.6.37:
push сценария из облака терял `trigger`/`end`). То же относится к
`sa02m-alice-config` — он тоже импортирует `sa02m_rules.store`.

OTA и оффлайн-пакет делают это сами (с 1.0.6.37): манифест обновления несёт
`sa02m-rules` в `services.restart[]` (обязательный v1 — исполняет и runner
1.0.5.66). Alice-семейство и мост — в опциональных `restart_if_*`; пакер
**не пишет** эти ключи, пока `MIN_UPDATER` < 1.0.6.37, иначе валидаторы
релизов 1.0.5.69–1.0.6.36 отвергают пакет с `E_MANIFEST` (окно и его дом —
`docs/OFFLINE_UPDATE_PACKAGE_V1.md`). Онлайн-OTA
после установки 1.0.6.37 пишет полный набор (манифест собирает уже новый
runner на плате). Честный предел: обновление применяет runner/валидатор
ПРЕДЫДУЩЕГО релиза — условные рестарты Alice/моста срабатывают со следующего
после 1.0.6.37 обновления; оффлайн-пакет с замороженным v1 принимается
платой ≥ 1.0.5.60 / runner ≥ 1.0.5.66, но **не включает ни одного юнита**
(`sa02m-dns-ensure.service` включает bootstrap-шим `sa02m-eth-coldboot.sh`).
С 1.0.6.39 runner сообщает свою версию (`UPDATER_VERSION`, гейт
`min_updater`) не штампом `1.0.5.66` — только поэтому порог `MIN_UPDATER`
вообще можно поднять (иначе `E_COMPAT` на каждой плате). С 1.0.6.40 источник
один — `/var/lib/sa02m-update/runner.version`: релиз, который установил
runner. Его пишут все точки установки runner'а (`03-webserver.sh`,
`update-www-only.sh` при наличии `etc/`, сам runner при OTA / пакете — через
журнал, так что откат возвращает старый штамп вместе со старым бинарём).
Плата без штампа (до 1.0.6.40) читает `www/network_config/VERSION`, без
обоих — пол `1.0.5.66`. Оговорка сузилась: только доставка `www/` **без**
`etc/` на плату без штампа завышает версию runner'а, и только до следующего
полного деплоя (`install.sh` / OTA / пакет) — тот пишет штамп. Гейт:
`runner-version-stamp` + `py-unit-update` + `update-deploy-skip`. Откат
после провала health-гейта (с 1.0.6.39) повторяет `daemon-reload` и наборы
рестартов манифеста на восстановленных файлах — до этого юниты держали в
памяти НОВЫЙ код поверх СТАРЫХ файлов до перезагрузки.
...
рабочая выжимка. Целевое состояние стенда «как 1.136» (службы, Alice,
Cloud, Docker) refresh **сам не выставляет** — таблица и чеклист:
`docs/bench-board-target-state.md` (runbook стенда, не контракт).
...
**Запуск НЕ из терминала (paramiko `exec_command`, `sa02m_remote.py exec`,
любой транспорт без tty).** Обёртку в таком случае запускают отдельной
сессией и с закрытым stdin, иначе закрытие SSH-канала может снять всю
процессную группу вместе с идущей установкой:

```
setsid nohup bash /tmp/sa02m-upd/scripts/offline-full-update.sh --unattended \
    --log /root/install-offline-<ver>.wrapper.log </dev/null >/dev/null 2>&1 &
```

Дальше состояние смотрят отдельным вызовом `… offline-full-update.sh --status`.
Именно эта форма довела до конца стенд 1.136 08.09.2026 в 22:48 после того, как
запуск через `nohup … &` в 22:07 оборвался; сам `install.sh` обёртка с 1.0.6.41
всегда запускает через `setsid … </dev/null` (видно в `--dry-run`).
...
уроненной обновлением службы (FAIL). Исключение сильнее снимка (с 1.0.6.41):
core-служба, юнит которой **замаскирован** или чей файл в `/etc/systemd/system`
**пуст** (обрыв установки), — всегда FAIL, даже если до обновления она была
неактивна: сама она уже не поднимется. Поставить/обновить и сторонние:
...
**Честно:** собственная телеметрия платы (id — имя платы, см.
`docs/MQTT_TOPICS.md`) появится снова после старта моста — это самоотчёт
службы о себе, а не «след» удалённого устройства.
...
### 12. Первая загрузка клона: гарантия суперблока и восстановление «кирпича»

**Гарантия (с 1.0.6.47).** Первый старт клона делает `resize2fs` на
смонтированном корне, и ядро платы (`6.1.0-rc6`) оставляет первичный суперблок
ext4 на eMMC с **устаревшей контрольной суммой** до следующей чистой
перезагрузки (ошибка ядра, исправленная в upstream: «ext4: fix bad checksum
after online resize»; запись — `docs/bugs/BUGLOG.md` 2026-09-16). Если в этом
окне снять питание, следующая загрузка молчит: ядро отказывается монтировать
корень. Поэтому `sa02m-rootfs-expand` до выставления `DONE` принудительно
заставляет ядро переписать суперблок (`fsfreeze -f /` + `-u /` под `timeout`) и
**проверяет сумму прямо на носителе** (чтение в обход кеша страниц). Вердикт
(с 1.0.6.48) — одна строка в **`/var/lib/sa02m-rootfs-expand.result`**,
принудительно сброшенная на eMMC (`fsync`), поэтому она переживает снятие
питания; это и есть улика, которую проверяет оператор:

- `<время> OK stored=0x… computed=0x… attempts=1` — гарантия выполнена; питание
  снимать можно в любой момент (`attempts=2` — потребовался один повтор,
  итог тот же).
- `<время> BAD … attempts=2` + файл `/var/lib/sa02m-rootfs-expand.csum-bad` +
  юнит `sa02m-rootfs-expand` в состоянии `failed` — гарантия НЕ выполнена. Плату
  **нельзя** обесточивать до одной чистой перезагрузки (`systemctl reboot`);
  после неё маркер и вердикт остаются как сигнал для аудита — снять вручную и
  разобраться, почему freeze не помог. Ни один демон маркер не читает: это улика
  для инструмента восстановления (копирует её в `repair-N/`), аудита и человека.

Когда ждать вердикт: на плате, где корень уже занимает всю eMMC, — через
несколько секунд после старта; на плате с ресайзом — **после строки `done:` в
логе** (`resize2fs` занимает 1–3 мин, затем freeze-цикл), не по секундомеру.

Лог `/var/log/sa02m-rootfs-expand.log` (строка `primary superblock checksum
OK …`) дублирует вердикт, пока плата не обесточена, и только пока. Корень
смонтирован `commit=600` с `journal_data_writeback`, поэтому всё, что записано в
`/var/log` и в журнал systemd за последние ≤10 мин и не сброшено `fsync`, после
снятия питания читается как файл нулевой длины (стенд 2026-09-16:
`sa02m-rootfs-expand.log`, `sa02m-reboot-reason.log` и persistent-журнал —
0 байт, journald ушёл в `/run/log/journal`; запись — `docs/bugs/BUGLOG.md`
2026-09-16 16:40). Скрипт сбрасывает `fsync`-ом свой вердикт, `DONE` и свой лог
— чужие файлы и журнал он не спасает. Смена политики монтирования — решение
Оператора (`.ai-dev/backlog.md`, [HIGH] 2026-09-16).

Платы в поле, пережившие хотя бы одну чистую перезагрузку после первого старта,
не затронуты: ядро уже переписало суперблок.

**Проверка образа перед серией** (по желанию, одна плата, ~15 мин): залить,
дождаться `OK` в `/var/lib/sa02m-rootfs-expand.result`, снять питание на
+2 мин, холодный старт — плата обязана подняться, и вердикт на ней читается
после старта (`cat /var/lib/sa02m-rootfs-expand.result`).

**Восстановление уже окирпиченной платы — без перезаливки, данные сохраняются.**
Симптом: первая загрузка была (ping), после снятия питания — ни линка, ни
консоли. Инструмент: `tools/imaging/autorun-repair-rootfs.sh` (описание и
ограничения — в его заголовке; никогда не пишет образ, меняет только метаданные
p2 через `e2fsck`).

1. Ввести плату в FEL и загрузить приёмник (initramfs) с USB-носителя, как для
   заливки (`tools/imaging/README.md`, вариант C — ручной autorun). Носитель
   должен быть доступен на запись: инструмент кладёт улики рядом с собой.
2. Скопировать `tools/imaging/autorun-repair-rootfs.sh` на носитель и на консоли
   приёмника выполнить `sh /mnt/autorun-repair-rootfs.sh` (p2 при этом НЕ
   смонтирован — инструмент сам откажется работать на смонтированном разделе).
3. Инструмент делает дампы «до», два прохода `e2fsck -fy` (первый только
   переписывает первичный суперблок из резервной копии и выходит с кодом 12
   «unable to set superblock flags» — это ожидаемо; второй выполняет полную
   проверку), дампы «после», монтирует p2 только для чтения и копирует журнал
   systemd, вердикт `sa02m-rootfs-expand.result` и маркеры первого старта в
   `repair-N/` на носителе.
4. Вердикт в `repair-N/collector.log`: `REPAIRED: e2fsck -n -f is clean` —
   вынуть носитель, выйти из FEL, включить питание: плата загружается в тот же
   rootfs. `NOT CLEAN after two passes` — проблема не только в суперблоке;
   сохранить `repair-N/` и перезаливать образ.

После восстановления первичный суперблок уже корректен; первый старт
повторно не выполняется (`DONE` на месте), гарантия выше к этой плате
применится при следующем чистом завершении работы так же, как к любой
работающей плате.
