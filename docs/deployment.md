...
`min_updater`) из развёрнутого `www/network_config/VERSION`, а не штампом
`1.0.5.66` — только поэтому порог `MIN_UPDATER` вообще можно поднять (иначе
`E_COMPAT` на каждой плате). Оговорка: `scripts/update-www-only.sh` обновляет
`VERSION` без runner'а — такая плата **завышает** версию runner'а; полный
деплой (`install.sh` / OTA / пакет) снова приводит их в соответствие. Откат
