...
**Важно:** `update-www-only.sh` НЕ разворачивает демон прошивальщика
(`opt/sa02m-flasher/`), tmpfiles и прочее из `etc/`. Если релиз менял их —
нужен `install.sh` (полный) ИЛИ отдельная доставка `opt/`/`etc/` + рестарт
`sa02m-flasher`.
