...
| **web-update (OTA)** | штатное самообновление с интернетом | вкладка «Обновление» → GitHub (`web_update_*.cgi`, semver); apply через shared runner при наличии. С 1.0.6.52 раннер работает transient-юнитом `sa02m-update-apply-<txn8>` (не в cgroup fcgiwrap), а перезагрузка платы на `verifying` **завершает** обновление (`sa02m-update-verify.service` после nginx), а не откатывает его — гарантии G1–G5 в `docs/contracts/web-update.md` «Жизненный цикл apply»; что из этого действует уже в доставляющем обновлении — таблица ниже. С 1.0.6.54 цикл раскладки файлов в раннере не запускает интерпретатор на каждый файл (до этого — ~2 100 запусков `python3` на 505 файлов, 33 мин на плате): **обновление, доставляющее 1.0.6.54, — последнее медленное** (его ещё выполняет старый раннер); со следующего apply тех же ~505 файлов идёт минуты, а не полчаса (цель ≤ 3 мин на стенде — `CHANGELOG.md` 1.0.6.54; подтверждается прогоном на стенде, не харнессом) |
...
- **конфигурация nginx (`etc/nginx/network_config.conf`) — GitHub-OTA её не несёт.**
  Это единственный дом списка «какой путь деплоя доставляет site-файл nginx»
  (контракты и модель угроз ссылаются сюда, не перечисляют заново):
  - **полная установка / `--refresh`** — `install.sh` → `scripts/03-webserver.sh`
    рендерит шаблон в `/etc/nginx/sites-available/network_config`;
  - **www-only** — `scripts/update-www-only.sh` из чекаута, в котором есть
    `scripts/11-devices.sh` **и** `opt/sa02m-devices/sa02m_devices/`: он вызывает
    `11-devices.sh`, а тот рендерит тот же шаблон, перелинковывает `sites-enabled`
    и делает `nginx -t && reload` (`update-www-only.sh:431-435`, `11-devices.sh:94-101`);
    чекаут без этих двух путей site-файл не трогает;
  - **офлайн-пакет `.sa02m`** — `scripts/offline-update-deploy-map.json` →
    `/etc/nginx/sites-available/network_config`;
  - **золотой образ**;
  - **GitHub-OTA («Обновление веб») — нет**: в `map_dst` раннера нет ветки для
    `etc/nginx/` (`DST_RE` лишь разрешает такое назначение), и голая ветка не
    помогла бы — файл в репо **шаблон** с `__PORT__`/`__WEB_ROOT__`, его нужно
    рендерить, как делают `03-webserver.sh` и `11-devices.sh`. Плата, поднятая
    только из панели, живёт со старым site-файлом (стенд 1.135, 2026-09-24 —
    срез `X-SA02M-Auth` из 1.0.6.53 до неё не дошёл). Расширение карты OTA на
    `etc/nginx/` (с рендером) — решение Оператора;
...
- После снятия золотого образа стенд-донор остаётся в состоянии «идентичность
  стёрта»: SSH host-ключей нет, пока `regen-ssh-host-keys` не отработает на
  следующей загрузке или пока плату не перезальют (§9 «Подготовка золотого
  образа»); до этого по SSH на него не зайти — это норма, а не поломка.
...
### Программный сторож после установки (с 1.0.6.51)

Любая установка (`install.sh`, refresh, офлайн-полное обновление) ставит и
запускает `sa02m-userspace-watchdog`, в том числе на платах, которые раньше
жили без него (обновлялись только refresh'ем). Сторож перезагружает плату, если
одна из его проверок подряд не проходит 10 мин (60 × 10 с): нет процесса
`nginx`/`sshd`, закрыт порт 22 или 9999, не отдаётся `/login.html`, `eth0` с
линком, но не поднят, load1 > 24, MemAvailable < 16 МиБ
(`etc/sa02m-userspace-watchdog.sh`).

**Что защищает саму установку — только блокировка** `/run/sa02m-imaging.lock`:
`install.sh` держит её весь прогон, и сторож на это время ставит проверки на
паузу. Льгота сторожа в 180 с считается **от загрузки**, а не от установки: на
плате, работающей дни, её уже нет.

**Установка, после которой nginx (или страница входа) не поднялся, даёт цикл
перезагрузок:** загрузка → 180 с льготы → 10 мин неудач → перезагрузка, то есть
примерно раз в 13 мин плюс время загрузки, пока причина не устранена.
Перезагрузка сама nginx не чинит. Выход — по SSH в любом окне между
перезагрузками:

1. Поставить сторожа на паузу до следующей загрузки:
   `touch /run/sa02m-imaging.lock` (он проверяет файл на каждом круге).
2. Разобраться по `/var/log/sa02m_userspace_watchdog.log` (какая проверка и
   почему) и `journalctl -u nginx -b`; починить.
3. Повторить установку. Она сама не возьмёт чужую блокировку и не снимет её
   (WARN «уже держит другой процесс»), поэтому после успешного прогона снять
   паузу руками: `rm -f /run/sa02m-imaging.lock` (или просто перезагрузить —
   `/run` это tmpfs). Порядок шагов важен: сторож на паузе **не обнуляет**
   счётчики провалов, поэтому снятый до починки замок вернёт перезагрузку через
   секунды, а не через 10 минут.

`systemctl disable --now sa02m-userspace-watchdog` тоже останавливает цикл, но
только до следующей установки: юнит инфраструктурный, установщик включает его
снова. Если вместе с nginx лежит и `sshd`, удалённого выхода нет (отладочной
консоли на COM-портах у платы нет — `scripts/01-system.sh`): цикл идёт до
физического вмешательства (офлайн-носитель/перезаливка).
...
(`/media/sdcard`) в образ eMMC **не входит**. На носителе каждый набор лежит в
своём каталоге `golden-<YYYYMMDD>/` рядом со своим `.sha256` (состав носителя —
`tools/imaging/README.md`), и каждый новый набор должен один раз пройти
«Проверку образа перед серией» (§12) до того, как с него льют серию.
...
- Слово вердикта бывает и другим: `NOCSUM` — у файловой системы выключен
  `metadata_csum`, проверять нечего, считается как `OK` (код 0); `ERR …` —
  суперблок не удалось прочитать (короткое чтение, нет магии ext4), считается как
  `BAD` (маркер + юнит `failed`). Источник слов — `primary_sb_checksum` и
  `ensure_primary_sb_checksum` в `etc/sa02m-rootfs-expand.sh`.
...
OK …`) дублирует вердикт, пока плата не обесточена, и только пока: на этом корне
(`commit=600` + `journal_data_writeback`) всё, что не сброшено `fsync`, после
снятия питания читается как файл нулевой длины — в том числе текстовые логи в
`/var/log` (стендовые улики — `docs/bugs/BUGLOG.md` 2026-09-16 16:40). Скрипт
сбрасывает `fsync`-ом свой вердикт, `DONE` и свой лог — чужие файлы он не
спасает. Журнал systemd с 1.0.6.51 устроен иначе: journald сам сбрасывает его на
eMMC `fsync`-ом раз в минуту (`SyncIntervalSec=1m`, политика —
`etc/systemd/sa02m-journald.conf`), а крючки RAM-журналирования Armbian, которые
уводили журнал в `/run`, выключены (`etc/default/armbian-ramlog`). По замыслу
снятие питания теряет не больше последней минуты журнала (`journalctl -b -1`);
**проверка на стенде — E-A**: до её результата это описание механизма, а не
проверенная гарантия. Механизм приходит на плату только с полной установкой
≥ 1.0.6.51 (`install.sh`: refresh или офлайн-полное обновление) или со
следующим золотым образом; OTA не несёт ни `etc/default/armbian-ramlog`, ни
drop-in journald, поэтому на плате, обновлённой только через OTA, журнал
остаётся как до 1.0.6.51 (крючки Armbian работают, синхронизация — умолчание
journald, 5 мин), и пункт 14 `scripts/dev/verify-release-on-board.sh` на ней
красный до полной установки. Смена политики монтирования — решение Оператора
(`.ai-dev/backlog.md`, [HIGH] 2026-09-16).

**`/var/log.hdd` — что можно удалить вручную.** Там лежат копии `/var/log`,
которые ночной `armbian-ramlog write` делал до 1.0.6.51 (на стенде 1.135 —
478 МБ, в том числе единственная копия syslog за 6–20.09); установщик каталог не
трогает. **Каталог `/var/log.hdd/journal` не удалять никогда**: на 1.135
`/var/log/journal` — ссылка в него, то есть это и есть постоянный журнал.
Перед удалением проверить, какие ссылки из `/var/log` указывают внутрь
`/var/log.hdd`, и не трогать их цели:

```bash
find /var/log -maxdepth 1 -type l -exec sh -c 'printf "%s -> %s\n" "$1" "$(readlink -f "$1")"' _ {} \;
```

Всё в `/var/log.hdd`, на что такая ссылка не указывает (текстовые логи и их
ротации — `syslog*`, `*.1`, `*.gz`), — копии прошлых ночей: их удаление журнал
не затрагивает, но стирает единственную копию старой истории (syslog за
6–20.09 на 1.135) — удалять осознанно.
