...
| Завершение БЕЗ перезагрузки | **да, вручную:** `scripts/sa02m-update-remedy.sh` → `runner reclaim` (целое дерево уходит в `sa02m-update-verify`, который запускается сразу) | «Ремонт застрявшего обновления» ниже |
...
дозавершается с перезапусками, целое дерево уходит в `sa02m-update-verify`);
со старым — `recover` во время работы и явное восстановление сторожа по
политике `sa02m-watchdog.conf`.

**Ожидаемый вывод** (проверено на 1.135):
