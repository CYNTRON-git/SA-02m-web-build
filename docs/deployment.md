...
снятия питания читается как файл нулевой длины — в том числе `/var/log` и журнал
systemd (стендовые улики — `docs/bugs/BUGLOG.md` 2026-09-16 16:40). Скрипт
сбрасывает `fsync`-ом свой вердикт, `DONE` и свой лог — чужие файлы и журнал он
не спасает. Смена политики монтирования — решение Оператора
