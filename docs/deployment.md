...
- **конфигурация nginx (`etc/nginx/network_config.conf`) — только офлайн-пакетом,
  не GitHub-OTA**: офлайн-карта кладёт её в `/etc/nginx/sites-available/network_config`,
  а в `map_dst` раннера ветки для `etc/nginx/` нет (`DST_RE` лишь разрешает такое
  назначение). Правка nginx (например, срез `X-SA02M-Auth` из 1.0.6.53) доходит до
  платы полной установкой, офлайн-пакетом или образом — плата, поднятая из панели,
  живёт со старым site-файлом (стенд 1.135, 2026-09-24). Расширение карты OTA на
  `etc/nginx/` — решение Оператора;
