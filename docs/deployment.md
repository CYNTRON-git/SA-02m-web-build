...
`sa02m-rules` в `services.restart[]`, Alice-семейство (`sa02m-alice-client`,
`sa02m-alice-config`, `sa02m-cloud-control`) — в `services.restart_if_active[]`
(рестарт только активного юнита, остановленный никогда не стартуется), а мост —
в `services.restart_if_changed` (рестарт активного моста и только когда пакет
моста реально изменился по журналу apply — port-lease RS-485). Честный предел:
обновление применяет runner/валидатор ПРЕДЫДУЩЕГО релиза, поэтому набор
рестарта срабатывает начиная со следующего после 1.0.6.37 обновления, а
оффлайн-пакет ≥ 1.0.6.37 принимает только плата уже на ≥ 1.0.6.37 (старый
валидатор отклоняет неизвестные ключи — `E_MANIFEST`; см.
`docs/OFFLINE_UPDATE_PACKAGE_V1.md`).
