...
> СА-02м — программируемый контроллер на базе одноплатного компьютера (Orange Pi и аналоги) под управлением Armbian Linux. Устройство, как правило, **не имеет доступа в интернет**, поэтому ниже описаны оба способа установки.

### Подготовка

Убедитесь, что у вас есть:

- Физический доступ к СА-02м **или** подключение по сети
- SSH-клиент на ПК (PuTTY, Windows Terminal, любой терминал)
- IP-адрес устройства (по умолчанию `192.168.1.136`)
- Пользователь с правами `sudo` (стандартно: `root` или `hmi`)

Подключитесь к устройству по SSH:

```bash
ssh root@192.168.1.136
# или если стандартный пользователь armbian:
ssh hmi@192.168.1.136
```

---

### Способ 1 — через интернет (устройство онлайн)

Если на СА-02м есть доступ в интернет (например, временно подключён кабель с выходом в сеть):
...
git clone https://github.com/CYNTRON-git/web.git /tmp/sa02m-web
cd /tmp/sa02m-web

# 3. Запустить установщик
sudo ./install.sh --ip 192.168.1.136 --pass cyntron
```

---

### Способ 2 — без интернета (перенос с ПК)

Это основной способ для СА-02м в изолированной сети.

#### Шаг 1. Скачайте репозиторий на ПК

```bash
git clone https://github.com/CYNTRON-git/web.git
```

Или скачайте ZIP через GitHub: **Code → Download ZIP**, распакуйте.

#### Шаг 2. Скопируйте файлы на устройство через SCP

**Linux / macOS (терминал):**

```bash
scp -r ./web root@192.168.1.136:/tmp/sa02m-web
```
...
Подключитесь по SFTP (`192.168.1.136`, порт `22`, пользователь `root`), скопируйте папку `web` в `/tmp/sa02m-web`.

#### Шаг 3. Установите зависимости с локального зеркала или вручную

> Если устройство полностью изолировано и `apt` не работает — установите пакеты заранее через другой компьютер в той же сети с доступом в интернет, либо заранее скачайте `.deb`-файлы.

Минимальный набор пакетов (должны быть установлены на устройстве):
...
После переноса файлов на устройство:
...
# Запустить установку
# Укажите IP, который уже назначен eth0 (или нужный вам)
sudo ./install.sh --ip 192.168.1.136 --mask 255.255.255.0 --gw 192.168.1.1 --pass cyntron
```
...
# Проверить доступность с самого устройства
...
1. Откройте браузер на ПК в **той же сети**, что и СА-02м
2. Перейдите по адресу: `http://192.168.1.136:9999`
3. На странице входа введите:
   - Логин: `admin`
   - Пароль: `cyntron` (или тот, что указан в `--pass`)
4. После входа откроется **Dashboard** с метриками устройства в реальном времени

> **Совет:** Если браузер не открывает страницу — проверьте, что ПК находится в той же подсети (`192.168.1.x`), и что порт `9999` не блокируется брандмауэром.

---

### Смена пароля после установки
