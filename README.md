...
>
> Если устройство **без интернета** — пакеты должны быть установлены заранее (например, из базового образа). Установщик проверит их наличие.
...
> При установке из ZIP-архива (не `git clone`) появится `[WARN] Нет git` — это нормально, функционал не затрагивает.  
