...
- Репозиторий прошивок:
  ...
  - **Один образ на всю линейку MR-02м:** сравнение «есть обновление» — по полю
    `version` в манифесте (`GET /firmware` → `latest_stable_version`), а не по
    сопоставлению сигнатуры модуля с записью прошивки; прошивка допускается только
    для сигнатур MR/MP-02м (либо с флагом обхода whitelist в UI для отладки).
