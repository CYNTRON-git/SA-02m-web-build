...
- **Исправлена авторизация демона RS-485 (тост «Порты: HTTP 401»).** CGI-слой
  (`lib_web_auth.sh`) хранил файл сессии по «сырому» токену, пустым, с правами
  600, а демон `sa02m-flasher` (`auth.py`) ищет файл по `sha256(token)`, читает
  срок из содержимого и читает по группе www-data (640). Из-за рассинхрона
  контракта демон отклонял любую валидную веб-сессию → `/api/flasher/*` всегда
  401. Приведено к документированной модели `lib_web_session.sh`: имя =
  `sha256(token)`, содержимое `<expiry> <user>`, каталог setgid + файлы 640.
  Побочно устранены редкие ложные 401 от смешанного хранилища сессий. Токен
  больше не виден как имя файла (security).
- **Истёкшая сессия → страница входа.** Вместо тостов «HTTP 401: unauthorized»
  ...
  лёгким запросом — редирект только при подтверждённой пропаже сессии, иначе
  исходный GET тихо повторяется.
