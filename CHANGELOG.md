...
- **Относительные пути ресурсов и API — устройство нормально открывается через
  облако.** Раньше все пути в UI были корне-абсолютными (`/static/…`,
  `/cgi-bin/…`), поэтому под облачным префиксом `/devcfg/<id>/` браузер запрашивал
  `cloud.cyntron.ru/static/…` → 404 (страница ломалась), а «спасал» лишь хрупкий
  307-редирект по `Referer` со стороны облака (лишний round-trip на каждый ресурс,
  а любой запрос без `Referer` — 404). Теперь у всех ссылок в
  `index.html`/`login.html`/`cloud.html` и у всех `fetch`/`location` в JS убран
  ведущий `/` — путь резолвится относительно страницы. Все страницы отдаются с
  корневой глубины (`/index.html`, `/login.html`, `/cloud.html`, без под-маршрутов),
  поэтому относительный путь одинаково верен и в LAN (`/`), и через облако
  (`/devcfg/<id>/`). Корневой переход `location.replace('/')` → `('./')` (текущая
  директория). `Location:`-заголовки CGI НЕ трогаем — их переписывает сам облачный
  прокси. Хрупкая `Referer`-зависимость устранена. Устаревшие прошивки в поле
  по-прежнему опираются на облачный 307-fallback — он остаётся.
