...
## 1.0.5.48 - Флешер: исправлено определение каналов по сигнатуре модуля (июл 2026)

### Флешер RS-485 (модули MR/MP-02m)

- **Исправлено определение количества каналов по строке сигнатуры** — при
  разборе сигнатуры устройства (рег. 290 / имя модуля) короткий ключ-подсказка
  (например «6DO») мог совпасть как подстрока раньше более длинного и точного
  ключа («6DO5DI2AO»), из-за чего для модуля «6DO5DI2AO» ошибочно определялись
  0 каналов DI и 0 каналов AO вместо 5 и 2. Разбор теперь ищет совпадение по
  самому длинному (наиболее специфичному) ключу, независимо от порядка в
  таблице подсказок — тем же способом исправлены Python (`sa02m-flasher`) и
  JS (веб-флешер), таблицы синхронизированы.
  **файлы:** `opt/sa02m-flasher/sa02m_flasher/module_profiles.py`,
  `www/network_config/static/js/flasher.js`
- **Восстановлена поддержка короткого алиаса «10DI»** для модуля «10DICON»
  (используется в списках алиасов моста MQTT) — при исправлении первой
  проблемы был случайно потерян неточный fallback-разбор, на который этот
  алиас опирался; добавлен явным ключом.
