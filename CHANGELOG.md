...
- **Управление дискретными выходами из вкладки MQTT.** В карточках всех типов
  ...
  кнопка-тумблер ВКЛ/ВЫКЛ (стиль блока DO главной страницы). Нажатие
  подтверждается только реальным эхом модуля с шины (~0,3–2 с); на время
  операции кнопка блокируется, при таймауте/недоступном брокере — понятный
  toast и возврат состояния. Для ДТВ — управление зуммером и светодиодами.
