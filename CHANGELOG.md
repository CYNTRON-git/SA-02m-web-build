...
## 1.0.5.16 - Установка/удаление служб CODESYS, MPLC4, Node-RED (июл 2026)

### Управление → Службы
- **Кнопки «Установить» / «Удалить»** для CODESYS, MPLC4 и Node-RED рядом с
  «Пуск»/«Стоп». Три состояния строки: не установлено → [Установить];
  установлено и работает → [Стоп][Удалить]; установлено и остановлено →
  [Пуск][Удалить]. Кнопка «Удалить» — в опасном (красном) стиле, оба темы,
  контраст AA.
- **«Удалить» = полная очистка** — остановка службы, purge пакета и удаление
  данных/конфигурации. Node-RED дополнительно удаляет пользователя `nodered`
  и его домашний каталог, но **оставляет Node.js**. Установочные файлы под
  `/opt/vendor-installers/<служба>/` **сохраняются** (переустановка без pscp).
- **Источники установки:** CODESYS и MPLC4 — только локальные файлы из
  `/opt/vendor-installers/`. Node-RED — онлайн (последняя версия) при наличии
  интернета, иначе оффлайн из подготовленного пакета.
- Удаление CODESYS лечит «отравление» apt (снимает hold, purge через `dpkg`,
  не `apt`, без `apt-get -f install`/`autoremove`).
...
### Установка / инфраструктура
- Sudoers для www-data расширен глобами `sa02m-web-service-ctl.sh install *` и
  `... uninstall *` (без выдачи apt/dpkg/systemctl напрямую).
  - **файлы:** `scripts/03-webserver.sh`
- Сборка rootfs дополнительно раскладывает systemd drop-in CODESYS и плагин
  `mplc_cyntron.so` в каталоги `/opt/vendor-installers/`.
  - **файлы:** `tools/debian-rootfs/create-sa02m-rootfs.sh`
- **Развёртывание требует полного `install.sh` (03-webserver.sh):** обновление
  только `www/` НЕ доставит новые правила sudoers и логику ctl-скрипта.
- **Node-RED оффлайн:** требует заранее подготовленного пакета (Node armhf +
  собранное дерево node-red@3 + unit) под `/opt/vendor-installers/nodered/`;
  готовится отдельно. Онлайн-установка Node-RED на стенде пока не проверена
  (нет интернета на стенде).
