...
- **Загрузка .fw:** перезапись файла с тем же именем вместо ilename.2.fw; UI поднимает обновлённую запись вверх списка; авто-выбор после скачивания/загрузки.
- **i18n:** описания прошивок в списке через 	(); lasherRerenderFirmware при смене языка.
- **Окно настройки модуля:** вкладки сразу из сигнатуры сканирования (uildConfigSnapshotStubFromDevice); на «Сведения» плитки МК обновляются из panel-опроса (merge mcu).
